...
const GAMES_DATA = [
  {
    ...
    description:
      "Venture into the Land of Shadow with the newest expansion. Discover hidden secrets and battle fearsome new bosses.",
  },
  {
    id: 2,
    title: "Grand Theft Auto V",
    category: "Console Subs",
    type: "PS5 Premium Edition",
    originalPrice: 39.99,
    price: 19.99,
    stock: 150,
    rating: 4.8,
    ...
    description:
      "Includes the complete GTAV story experience, Grand Theft Auto Online, and the Criminal Enterprise Starter Pack for PS5.",
  },
  {
    id: 3,
    ...
    rating: 4.2,
    image:
      "https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/2195250/library_600x900.jpg",
    description:
      "A new era for The World's Game on Nintendo Switch: 19,000+ fully licensed players, 700+ teams, and 30+ leagues playing together.",
  },
  {
    id: 4,
    ...
    rating: 4.0,
    image:
      "https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/1938090/library_600x900.jpg",
    description:
  ...
  },
  {
    ...
    rating: 4.7,
    image:
      "https://images.unsplash.com/photo-1606318801954-d46d46d3360a?auto=format&fit=crop&q=80&w=600",
    description:
  ...
  },
  {
    ...
    description:
      "A new spy-thriller adventure for Cyberpunk 2077. Return as cyber-enhanced mercenary V and embark on a high-stakes mission.",
  },
  {
    id: 8,
  ...
  },
  {
    ...
    rating: 4.6,
    image:
      "https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/990080/library_600x900.jpg",
    description:
      "An immersive, open-world action RPG set in the world first introduced in the Harry Potter books.",
  },
  {
    id: 10,
    title: "Valorant Points (1000 VP) TR",
    ...
    rating: 4.9,
    image:
      "https://images.unsplash.com/photo-1542751110-97427bbecf20?auto=format&fit=crop&q=80&w=600",
    description:
  ...
  },
  {
    ...
    rating: 4.6,
    image:
      "https://images.unsplash.com/photo-1598550880863-4e8aa3d0edb4?auto=format&fit=crop&q=80&w=600",
    description:
      "Get Robux to purchase upgrades for your avatar or buy special abilities in games.",
  },
];
...
export default function App() {
  ...
  const [orders, setOrders] = useState<Order[]>(() => {
    try {
      const o = localStorage.getItem("ludex_orders");
  ...
  useEffect(() => {
    localStorage.setItem("ludex_orders", JSON.stringify(orders));
  }, [orders]);
  ...
  const [paymentMethodsList, setPaymentMethodsList] = useState([
    {
      id: "p1",
      name: "Zain Cash",
      accountDetails: "Wallet Number: 0770 123 4567",
    ...
    },
    {
      id: "p2",
      name: "Mastercard",
      accountDetails: "Direct link will be emailed.",
    ...
    },
    {
      id: "p3",
      name: "AsiaHawala",
      accountDetails: "Wallet Number: 0770 987 6543",
      surcharge_percentage: 0,
      active: true,
    },
    {
      id: "p4",
      name: "FIB",
      accountDetails: "IBAN: IQ12345678901234567890",
      surcharge_percentage: 0,
      active: true,
    },
  ]);
  ...
  const [selectedGameId, setSelectedGameId] = useState<number | string | null>(
    null,
  );
  ...
  const toggleAdminMenu = (
    menu: "catalog" | "marketing" | "sales" | "ledger" | "system",
  ) => setAdminMenuState((prev) => ({ ...prev, [menu]: !prev[menu] }));
  ...
  const [currency, setCurrency] = useState<"USD" | "IQD">(
    (localStorage.getItem("ludex_currency") as "USD" | "IQD") || "IQD",
  );
  useEffect(() => {
  ...
  const [gamesList, setGamesList] = useState(
    GAMES_DATA.map((g) => ({
      ...
      is_preorder: g.id % 5 === 0,
      release_date: g.id % 5 === 0 ? "2026-11-20" : null,
    })),
  );
  ...
  const [newFinancialForm, setNewFinancialForm] = useState({
    type: "income",
    amount: "",
    description: "",
    date: "",
    ...
    const fetchPublicData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setGamesList((prev) => {
            const base =
              Object.keys(prev).length && prev[0].title
                ? prev
                : GAMES_DATA.map((g) => ({
                    ...g,
                    active: true,
      ...
      try {
        ...
        if (user) {
          ...
          try {
            ...
            console.warn(
              "[Auth Hub] Error fetching attributes (likely missing scopes). Falling back to ID token.",
              attrErr,
            );
            ...
            if (!attributes.email) {
              ...
              console.error(
                "[Auth Hub] ID token missing email. Zombie session cleaned.",
              );
              await signOut();
          ...
          console.log(
            "[Supabase Sync] Syncing profile with backend for:",
            userId,
          );
          try {
            ...
            if (syncData.success && syncData.profile) {
              console.log(
                "[Supabase Sync] Profile synced from backend:",
                syncData.profile.email,
              );
              ...
              if (isMounted) {
                if (profileData.role === "BANNED") {
                  ...
                  if (
                    ...
                    console.log(
                      "[Onboarding Guard] Missing platforms. Enforcing onboarding flow.",
                    );
                    setActiveTab("onboarding" as any);
                  } else {
                    console.log(
                      "[Auth Hub] User ready. Continuing to the application.",
                    );
                  }
                }
              }
              ...
              console.error(
                "[Supabase Sync] Backend failed to sync profile:",
                syncData,
              );
              if (isMounted) {
                setUserProfile({
                  ...
                  role:
                    userEmail === "admin@ludex.com" ||
                    userEmail === "rhydr4702@gmail.com" ||
                    userEmail === "abutrabali40@gmail.com"
                      ? "ADMIN"
                      : "USER",
                  platforms: ["PC"],
                  interests: [],
  ...
  const [userChatSessionId, setUserChatSessionId] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const fetchChatData = async () => {
      ...
      try {
        if (adminTab === "support" && userProfile?.role === "ADMIN") {
          const token = await getAuthToken();
          let res = await fetch("/api/chat/sessions", {
            headers: { Authorization: `Bearer ${token}` },
          });
          ...
          if (activeChatSessionId) {
            let msgRes = await fetch(
              `/api/chat/messages/${activeChatSessionId}`,
              { headers: { Authorization: `Bearer ${token}` } },
            );
            if (msgRes.ok) {
              const newHistory = await msgRes.json();
              setChatHistory((prev) => {
                const serverHistoryContents = [
                  ...newHistory.map((m: any) => m.content),
                ];
          ...
          if (userChatSessionId) {
            let msgRes = await fetch(
              `/api/chat/messages/${userChatSessionId}`,
              { headers: { Authorization: `Bearer ${token}` } },
            );
            if (msgRes.ok) {
              const newHistory = await msgRes.json();
    ...
    if (
      (adminTab === "support" && userProfile?.role === "ADMIN") ||
  ...
  const submitReview = async () => {
    ...
    const hasBought = orders.some(
      (o) => o.gameId === selectedGameId && o.status === "Approved",
    );
    if (!hasBought) {
      setToastMessage(
        language === "ar"
          ? "فشل - يجب عليك شراء المنتج أولاً لتقييمه"
          : "Failed - You must buy this product first to review it.",
      );
      return;
    }
      ...
      prev.map((g) =>
        g.id === id
          ? {
              ...g,
              views_count:
                (typeof g.views_count === "number" ? g.views_count : 0) + 1,
            }
          : g,
      ),
    );
  ...
  const getTier = (xp: number) => {
    ...
    if (xp < b)
      return { name: "Bronze", color: "text-orange-400", threshold: b };
  ...
  const xpPercentage =
    currentTier.name === "Diamond"
      ? 100
      : Math.min(
          ((userProfile?.xp_points || 0) / currentTier.threshold) * 100,
          100,
        );
  ...
  const displayPrice = (priceInUsd: number) => {
    return currency === "USD"
      ...
      : `${(priceInUsd * currencyRate).toLocaleString()} IQD`;
  };
  ...
  const isEligibleForLoyaltyDiscount =
    ...
    (approvedOrdersCount + 1) % loyaltyThreshold === 0;
  ...
  const calculateFinalPrice = (price: number) => {
    ...
    if (activePromoCode) {
      discountAmount +=
        (price - discountAmount) * (activePromoCode.discountPercent / 100);
    }

    const selectedPayment = paymentMethodsList.find(
      (p) => p.name === paymentMethod,
    );
    const surchargePercent = selectedPayment?.active
      ? selectedPayment.surcharge_percentage
      : 0;
  ...
  const applyPromoCode = () => {
    ...
    const code = promoCodes.find(
      (p) =>
        p.code.toLowerCase() === promoCodeInput.trim().toLowerCase() &&
        p.active,
    );
    if (!code) {
  ...
  const submitOrder = async () => {
      ...
      try {
        ...
        if (error) {
          console.warn(
            ...
            error.message,
            "- failing over to a local blob URL for demo.",
          );
        ...
        console.warn(err);
        console.warn(
          "Failed to connect to storage - failing over to a local blob URL for demo.",
        );
        receiptUrl = URL.createObjectURL(receiptFile);
      }
    }
    ...
    setToastMessage(
      language === "ar"
        ? "تم تقديم الطلب بنجاح! يرجى الانتظار للموافقة"
        : "Order submitted successfully! Awaiting admin approval.",
    );
    setTimeout(() => setToastMessage(null), 5000);
      ...
      try {
        ...
        if (res.ok) {
          ...
          const earnedXP = Math.floor(
            orderToApprove.amount * globalSettings.xpMultiplier,
          );
  ...
  const handleAdminReply = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!adminChatMessage.trim() || !userProfile?.id || !activeChatSessionId)
      return;
  ...
  if (activeTab === ("onboarding" as any)) {
    return (
      <div
        className="bg-[#050505] min-h-screen font-sans text-white flex flex-col items-center justify-center p-4 relative overflow-hidden"
        ...
        <div className="max-w-2xl mx-auto w-full flex flex-col items-center justify-center min-h-[60vh] gap-6 animate-in zoom-in-95 duration-500 relative z-10">
          <div className="text-center mb-4 flex flex-col items-center gap-2">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-2xl font-black tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-fuchsia-600">
                LUDEX
              </span>
            </div>
          </div>
          ...
          <div className="bg-[#0a0a0a] border border-purple-900/50 rounded-3xl p-8 md:p-12 w-full shadow-[0_0_80px_rgba(147,51,234,0.1)] flex flex-col gap-8">
            <div className="text-center space-y-3">
              ...
              <p className="text-gray-400 text-sm max-w-md mx-auto">
                {language === "ar"
                  ? "خطوة واحدة أخيرة قبل أن تتمكن من استكشاف المتجر وشراء الألعاب والوصول إلى العروض الحصرية."
                  : "One last step before you can explore the store, purchase games, and access exclusive deals."}
              </p>
            </div>
            ...
            <form
              ...
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  console.log(
                    "[Onboarding] Saving onboarding data for user:",
                    uId,
                  );
                  const token = await getAuthToken();
                  ...
                  if (res.ok) {
                    ...
                    console.error(
                      "[Onboarding] Supabase profile update error:",
                      await res.text(),
                    );
            ...
            >
                    ...
                    (plat) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setUserProfile({
                            ...userProfile,
                            platforms: curr.includes(plat)
                              ? curr.filter((p: string) => p !== plat)
                              : [...curr, plat],
                          });
                        }}
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-3">
                  {["Action", "RPG", "Strategy", "Sports", "Racing", "FPS"].map(
                    (int) => (
                      <button
                        type="button"
                        key={int}
                        onClick={() => {
                          ...
                          setUserProfile({
                            ...userProfile,
                            interests: curr.includes(int)
                              ? curr.filter((i: string) => i !== int)
                              : [...curr, int],
                          });
                        }}
              ...
              <div className="pt-4 flex flex-col gap-3">
                <button
                  type="submit"
                  className="w-full bg-purple-600 hover:bg-purple-500 text-white font-black uppercase tracking-widest py-4 rounded-xl transition-all shadow-[0_0_20px_rgba(147,51,234,0.3)] active:scale-[0.98] flex items-center justify-center gap-2 group"
                >
                  {language === "ar" ? "بدء اللعب" : "Start Playing"}
                  <ArrowRight
                    className={`w-5 h-5 group-hover:translate-x-1 transition-transform ${language === "ar" ? "rotate-180 group-hover:-translate-x-1" : ""}`}
                  />
                </button>
              </div>
            </form>
  ...
  return (
    <>
      <div
        dir={language === "ar" ? "rtl" : "ltr"}
        className="min-h-screen h-screen bg-[#050505] text-[#F3F4F6] font-sans flex flex-col overflow-hidden relative select-none pb-20 md:pb-0"
      >
        ...
        {activeTab === "admin" ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center relative z-20 h-full w-full bg-[#050505]">
              <div className="text-center space-y-6">
                ...
                <p className="text-gray-500 max-w-sm mx-auto px-4">
                  The route you are trying to access does not exist or you don't
                  have authorization.
                </p>
                <button
                  onClick={() => {
                  ...
                  className="mt-8 px-8 py-3 bg-[#111] border border-purple-900/50 rounded-xl text-white font-bold hover:bg-purple-900/20 transition-all flex items-center gap-2 mx-auto"
                >
                  Return to Home
                </button>
              </div>
            ...
            <div className="flex-1 flex overflow-hidden z-10 w-full h-full bg-[#050505]">
              ...
              <div className="w-64 bg-[#0a0a0a] border-e border-purple-900/40 p-4 flex flex-col gap-2 relative z-20 overflow-y-auto custom-scrollbar flex-shrink-0 h-full">
                <div className="px-4 py-6 border-b border-gray-800 mb-6 flex flex-col gap-2">
                  ...
                  <p className="text-[10px] text-green-400 mt-1 uppercase font-mono bg-green-500/10 px-2 py-1 rounded inline-block w-fit">
                    /ludex-hq-portal
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Package className="w-4 h-4" /> {t[language].catalogStock}
                    </div>
                    <ChevronDown
                  ...
                  {adminMenuState.catalog && (
                    <div className="pl-6 flex flex-col gap-1">
                      ...
                      >
                        ...
                        {t[language].adminSubs}
                      </button>
                      <button
                        onClick={() => setAdminTab("products")}
                        className={`flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-bold transition-all ${adminTab === "products" ? "bg-purple-600/20 text-purple-400 border border-purple-500/30" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
                      >
                        <ShoppingBag className="w-4 h-4" />
                        {t[language].adminProds}
                      </button>
                      <button
                        onClick={() => setAdminTab("games")}
                        className={`flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-bold transition-all ${adminTab === "games" ? "bg-purple-600/20 text-purple-400 border border-purple-500/30" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
                      >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <User className="w-4 h-4" /> {t[language].salesCrm}
                    </div>
                    <ChevronDown
                  ...
                  {adminMenuState.sales && (
                    <div className="pl-6 flex flex-col gap-1">
                      ...
                      >
                        <ShoppingBag className="w-4 h-4" />
                        {t[language].adminSales}
                      </button>
                      <button
                        onClick={() => setAdminTab("orders")}
                        className={`flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-bold transition-all ${adminTab === "orders" ? "bg-purple-600/20 text-purple-400 border border-purple-500/30" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
                      >
                        <ShoppingBag className="w-4 h-4" />
                ...
                <div className="flex flex-col gap-1">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <CreditCard className="w-4 h-4" /> {t[language].finLedger}
                    </div>
                    <ChevronDown
                  ...
                  {adminMenuState.ledger && (
                    <div className="pl-6 flex flex-col gap-1">
                      ...
                      >
                        ...
                        {t[language].adminTrans}
                      </button>
                      <button
                        onClick={() => setAdminTab("financials")}
                        className={`flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-bold transition-all ${adminTab === "financials" ? "bg-purple-600/20 text-purple-400 border border-purple-500/30" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
                      >
                        <CreditCard className="w-4 h-4" />
                        {t[language].adminFin}
                      </button>
                      <button
                        ...
                        className={`flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-bold transition-all ${adminTab === "payments" ? "bg-purple-600/20 text-purple-400 border border-purple-500/30" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
                      >
                        <CreditCard className="w-4 h-4" />
                        {t[language].adminPay}
                      </button>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Settings className="w-4 h-4" /> {t[language].sysTools}
                    </div>
                    <ChevronDown
                  ...
                  {adminMenuState.system && (
                    <div className="pl-6 flex flex-col gap-1">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-bold transition-all ${adminTab === "settings" ? "bg-purple-600/20 text-purple-400 border border-purple-500/30" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
                      >
                        <Settings className="w-4 h-4" />
                        {t[language].adminSet}
                      </button>
                      <button
                        ...
                        className={`flex items-center gap-3 px-4 py-2 rounded-xl text-xs font-bold transition-all ${adminTab === "users" ? "bg-purple-600/20 text-purple-400 border border-purple-500/30" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
                      >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Star className="w-4 h-4" /> {t[language].marketing}
                    </div>
                    <ChevronDown
                ...
                <div className="mt-auto border-t border-gray-800 pt-4 flex flex-col gap-2">
                  <button
                    ...
                    className="flex items-center gap-2 text-gray-500 hover:text-white transition-colors px-4 py-2 text-xs"
                  >
              ...
              <div className="flex-1 flex flex-col overflow-hidden relative z-10">
                <header className="h-20 border-b border-purple-900/40 bg-[#0a0a0a] px-8 flex items-center justify-between">
                  <h2 className="text-lg font-bold text-white capitalize">
                    {adminTab.replace(/([A-Z])/g, " $1").trim()}
                  </h2>
                  <div className="flex items-center gap-4">
                    <div className="w-8 h-8 rounded-full bg-purple-600 flex items-center justify-center font-bold text-xs shadow-[0_0_10px_#9333ea]">
                      AD
                    </div>
                  </div>
                </header>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        ...
                        <div className="bg-[#111] border border-gray-800 rounded-xl p-6 shadow-xl">
                          ...
                          <p className="text-4xl font-black text-red-500 mt-2">
                            {gamesList.filter((g) => (g.stock || 0) < 3).length}
                          </p>
                        </div>
                        <div className="bg-[#111] border border-gray-800 rounded-xl p-6 shadow-xl">
                          <p className="text-[10px] text-gray-500 uppercase font-bold tracking-widest flex items-center gap-2">
                            <User className="w-3 h-3 text-blue-400" />{" "}
                            {t[language].activeCustomers || "Active Users"}
                          </p>
                          ...
                          <table className="w-full text-left text-sm text-gray-400">
                            <thead className="bg-[#1a1a1a] text-xs uppercase font-bold border-b border-gray-800">
                              <tr>
                                ...
                                <th className="px-4 py-3">
                                  {t[language].passwordUpper}
                                </th>
                                <th className="px-4 py-3">
                                  {t[language].catCol}
                                </th>
                                <th className="px-4 py-3">
                                  {t[language].statusMsg}
                                </th>
                                <th className="px-4 py-3">
                                  {t[language].sellCount}
                                </th>
                                <th className="px-4 py-3">
                                  {t[language].actions}
                                </th>
                            ...
                            <tbody className="divide-y divide-gray-800">
                              {subList.map((item, index) => (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3">
                                    <span className="bg-gray-800 text-gray-300 px-2 py-1 rounded text-[10px] uppercase tracking-wider">
                                      {item.category}
                                    </span>
                                  </td>
                                  <td className="px-4 py-3">
                                    <span
                                      className={`px-2 py-1 rounded text-[10px] uppercase tracking-wider font-bold ${item.status === "active" ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"}`}
                                    >
                                      {item.status || "Active"}
                                    </span>
                                  </td>
                                  ...
                                  <td className="px-4 py-3 flex gap-2">
                                    ...
                                    <button
                                      ...
                                      className="text-purple-400 hover:text-purple-300 text-[10px] uppercase font-black bg-purple-500/10 px-2 py-1 rounded border border-purple-500/30 shadow-[0_0_10px_rgba(168,85,247,0.2)]"
                                    >
                                      {t[language].publishBtn}
                                    </button>
                                    <button
                                      onClick={() =>
                                        handleCrudDelete(
                              ...
                              {subList.length === 0 && (
                                <tr>
                                  ...
                                  >
                                    No subscriptions found. Click "Add New
                                    Account" to begin.
                                  </td>
                                </tr>
                              )}
                          ...
                          <table className="w-full text-left text-sm text-gray-400">
                            <thead className="bg-[#1a1a1a] text-xs uppercase font-bold border-b border-gray-800">
                              <tr>
                                ...
                                <th className="px-4 py-3">
                                  {t[language].costPriceCol}
                                </th>
                                <th className="px-4 py-3">
                                  {t[language].sellPriceCol}
                                </th>
                                <th className="px-4 py-3">
                                  {t[language].supplierCol}
                                </th>
                                <th className="px-4 py-3">
                                  {t[language].catCol}
                                </th>
                            ...
                            <tbody className="divide-y divide-gray-800">
                              {productList.map((item, index) => (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3 font-bold text-white max-w-[120px] truncate">
                                    {item.name}
                                  </td>
                                  ...
                                  <td className="px-4 py-3 flex gap-2 whitespace-nowrap">
                                    <button
                                      ...
                                      className="text-purple-400 hover:text-purple-300 text-[10px] uppercase font-black bg-purple-500/10 px-2 py-1 rounded border border-purple-500/30"
                                    >
                                      {t[language].publishBtn}
                                    </button>
                                    <button
                                    ...
                                    <button
                                      onClick={() =>
                                        handleCrudDelete("products", item.id)
                                      }
                                      className="text-red-400 hover:text-red-300 text-xs font-bold"
                                    >
                                      {t[language].deleteBtn}
                              ...
                              {productList.length === 0 && (
                                <tr>
                                  <td
                                    ...
                                    className="px-4 py-8 text-center text-gray-500 italic"
                                  >
                                    No products found in storefront.
                                  </td>
                                </tr>
                              )}
                            </tbody>
                                    ...
                                    {isEditing ? (
                                      <div className="flex flex-col gap-1">
                                        ...
                                        <input
                                          ...
                                          className="w-16 bg-black border border-gray-700 rounded px-1"
                                        />
                                        Disc:{" "}
                                        <input
                                          type="number"
                                          value={
                                            editingOrderForm.discountApplied
                                          }
                                          onChange={(e) =>
                                            setEditingOrderForm({
                                              ...editingOrderForm,
                                      ...
                                      <div>
                                        ...
                                        {order.discountApplied
                                          ? ` (-${displayPrice(order.discountApplied)})`
                                          : ""}
                                      </div>
                                    )}
                                  </td>
                                    ...
                                    {isEditing ? (
                                      <div className="flex flex-col gap-2">
                                              ...
                                              orders.map((o) =>
                                                o.id === order.id
                                                  ? {
                                                      ...o,
                                                      status:
                                                        editingOrderForm.status as any,
                                                      amount: parseFloat(
                                                        editingOrderForm.amount,
                                                      ),
                                                      discountApplied:
                                                        parseFloat(
                                                          editingOrderForm.discountApplied,
                                        ...
                                        <button
                                          ...
                                          className="bg-gray-600 hover:bg-gray-500 text-white px-2 py-1 rounded text-xs font-bold transition-colors"
                                        >
                                          Cancel
                                        </button>
                                      </div>
                                      ...
                                      <>
                                        ...
                                        {order.status === "Pending" && (
                                          <button
                                            onClick={() => {
                                              const gameItem = gamesList.find(
                                                (g) => g.id === order.gameId,
                                              );
                                              if (
                                                gameItem &&
                                                gameItem.stock !== undefined
                                              ) {
                                                gameItem.stock = Math.max(
                                                  0,
                                                  gameItem.stock - 1,
                                                );
                                                gameItem.sales_count =
                                                  (typeof gameItem.sales_count ===
                                                  "number"
                                                    ? gameItem.sales_count
                                                    : 0) + 1;
                      ...
                      <div className="bg-[#111] border border-gray-800 rounded-xl p-6 flex flex-col gap-6">
                        <div className="flex justify-between items-center border-b border-gray-800 pb-4">
                          <h3 className="text-white font-bold text-lg flex items-center gap-2">
                            ...
                            {t[language].manageInv}
                          </h3>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          <div className="bg-black border border-gray-800 rounded-xl p-6">
                            <h4 className="text-sm font-bold text-gray-300 mb-6 uppercase tracking-widest">
                              Add New Game
                            </h4>
                            <div className="space-y-4">
                              <input
                                ...
                                className="w-full bg-[#111] border border-gray-800 rounded-lg p-3 text-sm focus:outline-none focus:border-purple-500 text-white"
                              />
                              ...
                              />
                              <textarea
                                placeholder="Description (Supports Rich Text HTML)"
                                ...
                                className="w-full bg-[#111] border border-gray-800 rounded-lg p-3 text-sm focus:outline-none focus:border-purple-500 text-white min-h-[120px]"
                              ></textarea>
                              <button
                                onClick={() => {
                                  if (newGameForm.title && newGameForm.price) {
                                    if (editingGameId) {
                                        ...
                                        gamesList.map((g) =>
                                          g.id === editingGameId
                                            ? {
                                                ...
                                                description:
                                                  newGameForm.description,
                                                stock:
                                                  parseInt(newGameForm.stock) ||
                                                  0,
                                              }
                                            : g,
                                        ),
                                      ...
                                      const newGame = {
                                        ...
                                        image:
                                          newGameForm.image ||
                                          "https://images.unsplash.com/photo-1605901309584-818e25960b8f?auto=format&fit=crop&q=80&w=600",
                          ...
                          <div className="flex flex-col gap-4 max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-800 pr-2">
                            {gamesList.map((game) => (
                              <div
                                key={game.id}
                                className={`bg-black border ${!game.active ? "border-red-900/30 opacity-60" : "border-gray-800"} rounded-lg p-4 flex gap-4 items-center transition-all`}
                              >
                                <img
                                  src={game.image}
                                ...
                                <div className="flex-1">
                                  <h5 className="text-sm font-bold text-white flex items-center gap-2">
                                    {game.title}
                                    {!game.active && (
                                      <span className="px-2 py-0.5 bg-red-900/30 text-red-500 rounded text-[10px] uppercase">
                                        Archived
                                      </span>
                                    )}
                                  </h5>
                                ...
                                <button
                                  ...
                                  className={`${game.active ? "text-orange-500 hover:text-orange-400" : "text-green-500 hover:text-green-400"} p-2`}
                                >
                                  {game.active ? (
                                    <svg
                                      xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          ...
                          <div className="flex flex-col gap-4 max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-800 pr-2">
                            {categoriesList.map((cat) => (
                              <div
                                key={cat.id}
                                className={`bg-black border ${!cat.active ? "border-red-900/30 opacity-60" : "border-gray-800"} rounded-lg p-4 flex gap-4 items-center transition-all`}
                              >
                                <div className="flex-1">
                                  <h5 className="text-sm font-bold text-white flex items-center gap-2">
                                    {cat.name}
                                ...
                                <button
                                  ...
                                  className={`${cat.active ? "text-orange-500 hover:text-orange-400" : "text-green-500 hover:text-green-400"} p-2`}
                                >
                                  {cat.active ? (
                                    <svg
                                      xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        onChange={async (e) => {
                                          ...
                                          try {
                                            ...
                                            if (res.ok) {
                                              ...
                                              if (refErr)
                                                console.error(
                                                  "Supabase select error (refetching users 1):",
                                                  refErr,
                                                );
                                              if (refreshed)
                                      ...
                                      <button
                                        onClick={async () => {
                                          if (
                                            window.confirm(
                                              language === "ar"
                                                ? "هل أنت متأكد من حذف هذا المستخدم نهائيا؟ سيتم حذفه من AWS والسب بيس."
                                                : "Are you sure you want to permanently delete this user? It will be deleted from AWS and Supabase.",
                                            )
                                          ) {
                                            try {
                                              const token =
                                                await getAuthToken();
                                              const res = await fetch(
                                                `/api/admin/profiles/cognito/${user.id}`,
                                                {
                                                  method: "DELETE",
                                                  headers: {
                                                    Authorization: `Bearer ${token}`,
                                              ...
                                              if (res.ok) {
                                                ...
                                                if (refErr)
                                                  console.error(
                                                    "Supabase select error (refetching users 3):",
                                                    refErr,
                                                  );
                                                if (refreshed)
                                                  setUsersList(refreshed);
                      ...
                      <div className="bg-[#111] border border-gray-800 rounded-xl p-6 flex flex-col gap-6">
                        ...
                        <div className="bg-black border border-gray-800 rounded-xl p-4 flex gap-4 items-center">
                          <select
                            ...
                            className="bg-[#111] border border-gray-800 rounded p-2 text-white"
                          >
                          ...
                          <input
                            ...
                            className="bg-[#111] border border-gray-800 rounded p-2 text-white w-24"
                          />
                          <input
                            type="text"
                            placeholder="Description"
                            value={newFinancialForm.description}
                            onChange={(e) =>
                              setNewFinancialForm({
                                ...newFinancialForm,
                                description: e.target.value,
                            ...
                            className="bg-[#111] border border-gray-800 rounded p-2 text-white flex-1"
                          />
                          <input
                            type="date"
                            value={newFinancialForm.date}
                            onChange={(e) =>
                              setNewFinancialForm({
                                ...newFinancialForm,
                                date: e.target.value,
                          ...
                          <button
                            onClick={() => {
                              if (
                                newFinancialForm.amount &&
                                newFinancialForm.description
                              ) {
                                if (editingFinancialId) {
                                  setFinancialsList(
                                    financialsList.map((f) =>
                                      f.id === editingFinancialId
                                        ? {
                                            ...f,
                                            type: newFinancialForm.type as
                                              | "income"
                                              | "expense",
                                            amount: parseFloat(
                                              newFinancialForm.amount,
                          ...
                          >
                            {editingFinancialId ? "Update" : "Add Record"}
                          </button>
                          {editingFinancialId && (
                            <button
                              onClick={() => {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          <div className="bg-black border border-gray-800 rounded-xl p-6">
                            ...
                            <div className="space-y-4">
                              <input
                                type="text"
                                placeholder="Gateway Name (e.g. Zain Cash)"
                              ...
                              <input
                                type="text"
                                placeholder="Account Details (e.g. Wallet Number)"
                                value={newPaymentForm.accountDetails}
                                onChange={(e) =>
                                  setNewPaymentForm({
                                    ...newPaymentForm,
                              ...
                              <div>
                                ...
                                <input
                                  type="number"
                                  placeholder="0"
                                  value={newPaymentForm.surcharge_percentage}
                                  onChange={(e) =>
                                    setNewPaymentForm({
                                      ...newPaymentForm,
                                      surcharge_percentage: e.target.value,
                                    })
                                  }
                                  className="w-full bg-[#111] border border-gray-800 rounded-lg p-3 text-sm focus:outline-none focus:border-purple-500 text-white"
                                />
                                          ...
                                          p.id === editingPaymentId
                                            ? {
                                                ...
                                                surcharge_percentage:
                                                  parseFloat(
                                                    newPaymentForm.surcharge_percentage,
                                                  ) || 0,
                                              }
                                            : p,
                                        ),
                          ...
                          <div className="flex flex-col gap-4 max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-800 pr-2">
                            {paymentMethodsList.map((method) => (
                              <div
                                key={method.id}
                                className={`bg-black border ${!method.active ? "border-red-900/30 opacity-60" : "border-gray-800"} rounded-lg p-4 flex gap-4 items-start transition-all`}
                              >
                                <div className="flex-1">
                                  <h5 className="text-sm font-bold text-white flex items-center gap-2">
                                    {method.name}
                                    {method.surcharge_percentage > 0 && (
                                      <span className="px-2 py-0.5 bg-orange-900/30 text-orange-500 rounded text-[10px] font-mono">
                                        +{method.surcharge_percentage}% FEE
                                      </span>
                                    )}
                                    {!method.active && (
                                      <span className="px-2 py-0.5 bg-red-900/30 text-red-500 rounded text-[10px] uppercase">
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    setNewPaymentForm({
                                      ...
                                      surcharge_percentage:
                                        method.surcharge_percentage.toString(),
                                    });
                                  }}
                                  className="text-blue-500 hover:text-blue-400 p-2"
                                >
                                ...
                                <button
                                  ...
                                  className={`${method.active ? "text-orange-500 hover:text-orange-400" : "text-green-500 hover:text-green-400"} p-2 text-xs font-bold uppercase`}
                                >
                          ...
                          <table className="w-full text-left text-sm text-gray-400">
                            <thead className="bg-[#1a1a1a] text-xs uppercase font-bold border-b border-gray-800">
                              <tr>
                                ...
                                <th className="px-4 py-3">
                                  {t[language].finalPrice}
                                </th>
                                <th className="px-4 py-3">
                                  {t[language].customerUsername}
                                </th>
                                <th className="px-4 py-3">
                                  {t[language].notes}
                                </th>
                              ...
                              {salesList.map((item, index) => (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3 flex gap-2">
                                    ...
                                    <button
                                      onClick={() =>
                                        handleCrudDelete("sales", item.id)
                                      }
                                      className="text-red-400 hover:text-red-300 text-xs font-bold"
                                    >
                                      {t[language].deleteBtn}
                            ...
                            <tbody className="divide-y divide-gray-800">
                              {transList.map((item, index) => (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3">
                                    <span
                                      className={`px-2 py-1 rounded text-[10px] uppercase font-bold ${item.type === "income" ? "bg-green-500/10 text-green-400" : item.type === "expense" ? "bg-red-500/10 text-red-400" : "bg-gray-500/10 text-gray-400"}`}
                                    >
                                      {item.type}
                                    </span>
                                  </td>
                                  <td
                                    className={`px-4 py-3 font-mono ${item.type === "income" ? "text-green-400" : "text-red-400"}`}
                                  >
                                    ${item.amount}
                                  </td>
                              ...
                              {settingsList.map((item, index) => (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3 flex gap-2">
                                    ...
                                    <button
                                      onClick={() =>
                                        handleCrudDelete("settings", item.id)
                                      }
                                      className="text-red-400 hover:text-red-300 text-xs font-bold"
                                    >
                                      {t[language].deleteBtn}
                        ...
                        <div className="flex justify-between items-center border-b border-gray-800 pb-4">
                          ...
                          >
                            {editingSettings
                              ...
                              : t[language].editSettings}
                          </button>
                        </div>

                        <div className="grid grid-cols-2 gap-6">
                          <div>
                            <label className="block text-xs uppercase text-gray-500 mb-2 font-bold">
                              Store Name
                            </label>
                            {editingSettings ? (
                              <input
                          ...
                          <div>
                            <label className="block text-xs uppercase text-gray-500 mb-2 font-bold">
                              Currency Rate (1 USD to IQD)
                            </label>
                            {editingSettings ? (
                              <input
                                type="number"
                                value={settingsForm.currencyRate}
                                onChange={(e) =>
                                  setSettingsForm({
                                    ...settingsForm,
                                    currencyRate:
                                      parseFloat(e.target.value) || 1500,
                                  })
                                }
                                className="w-full bg-black border border-gray-700 rounded p-2 text-white"
                              />
                    ...
                    <div className="max-w-7xl w-full mx-auto h-[calc(100vh-250px)] min-h-[600px] flex gap-4 mt-6">
                      ...
                      <div className="w-[320px] shrink-0 bg-[#0a0a0a] border border-gray-800 rounded-2xl overflow-hidden flex flex-col shadow-xl">
                        <div className="p-5 border-b border-gray-800 bg-[#111] flex items-center justify-between">
                          ...
                          <span className="text-xs font-bold bg-gray-800 text-gray-300 px-2 py-1 rounded-md">
                          ...
                          {chatSessions?.map((session) => (
                            ...
                            >
                              {session.admin_unread > 0 && (
                                <span className="absolute top-3 right-3 w-2.5 h-2.5 bg-purple-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(168,85,247,0.8)]"></span>
                              )}
                              <img
                                src={
                                  session.profiles?.avatar_url ||
                                  `https://api.dicebear.com/7.x/avataaars/svg?seed=${session.profiles?.display_name || session.profiles?.email}&backgroundColor=4c1d95`
                                }
                                onClick={(e) => {
                                  e.stopPropagation();
                                  session.user_id &&
                                    handleViewProfile(session.user_id);
                                }}
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-center mb-0.5">
                                  <p className="font-bold text-sm text-gray-200 truncate group-hover:text-white">
                                ...
                                <p className="text-xs text-gray-500 truncate mt-0.5 pr-4">
                                  User ID: {session.user_id?.substring(0, 8)}...
                                </p>
                              </div>
                            </div>
                          ))}
                          ...
                          <>
                              ...
                              <div className="flex items-center gap-4">
                                <img
                                  src={
                                    ...
                                    `https://api.dicebear.com/7.x/avataaars/svg?seed=${chatSessions.find((s) => s.id === activeChatSessionId)?.profiles?.display_name || "User"}&backgroundColor=4c1d95`
                                  }
                            ...
                            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4 scrollbar-thin scrollbar-thumb-gray-800">
                              {chatHistory?.map((msg) => {
                                ...
                                const isAdminType =
                                  msg.sender?.role === "ADMIN" || isMyMsg;
                                return (
                                  <div
                                    key={msg.id}
                                    className={`flex flex-col max-w-[80%] ${isAdminType ? "self-end items-end" : "self-start items-start"}`}
                                  >
                                    <div className="flex items-center gap-2 mb-2">
                                      <span className="text-[10px] text-gray-500 font-bold">
                                        {isMyMsg ? (
                                          ...
                                          <button
                                            ...
                                            className="hover:text-purple-400 hover:underline transition-all"
                                          >
                                    ...
                                    <div
                                      className={`px-4 py-2.5 rounded-2xl text-[14px] shadow-sm whitespace-pre-wrap leading-relaxed ${
                                        isAdminType
                                          ...
                                          if (!game)
                                            return (
                                              <div className="flex flex-col gap-3 min-w-[200px]">
                                                <p className="text-xs font-bold opacity-80 text-purple-300">
                                                  Product Inquiry
                                                </p>
                                                <div className="bg-black/40 border border-red-500/20 rounded-lg p-2 flex gap-3 items-center">
                                                  <div className="w-12 h-12 flex items-center justify-center bg-red-900/20 rounded border border-red-500/30 text-red-400">
                                                    ?
                                                  </div>
                                                  <div className="flex-1 min-w-0">
                                                    <p className="font-bold text-gray-400 text-[13px] truncate">
                                                      Product Not Found
                                                    </p>
                                                    <p className="text-red-400 text-[10px] font-black">
                                                      ID: {gameId}
                                                    </p>
                                                  </div>
                                                </div>
                                            ...
                                            <div className="flex flex-col gap-3 min-w-[200px]">
                                              ...
                                              >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                  ...
                                                  <p className="text-purple-400 text-[10px] font-black">
                            ...
                            <div className="p-4 bg-[#0a0a0a] border-t border-gray-800">
                              <form
                                ...
                                className="flex gap-3 bg-[#111] p-2 rounded-xl border border-gray-800 focus-within:border-purple-500/50 transition-colors"
                              >
                                <input
                                  type="text"
                                  id="adminChatMessage"
                                  ...
                                  onChange={(e) =>
                                    setAdminChatMessage(e.target.value)
                                  }
                                ...
                                <button
                                  ...
                                  className="bg-purple-600 px-6 font-bold rounded-lg text-white hover:bg-purple-500 disabled:opacity-50 transition-colors flex items-center justify-center"
                                >
                                  Send
                                </button>
                              </form>
                      ...
                      {activeChatSessionId && (
                        <div className="w-[300px] shrink-0 bg-[#0a0a0a] border border-gray-800 rounded-2xl overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-gray-800 shadow-xl hidden lg:block">
                          <h3 className="font-bold text-white mb-6 border-b border-gray-800 pb-3 flex items-center gap-2">
                            <User size={18} className="text-gray-400" /> User
                            Info
                          </h3>
                          {(() => {
                            const session = chatSessions.find(
                            ...
                            return (
                              <div className="flex flex-col gap-4 text-sm">
                                <div>
                                  ...
                                  <p className="text-white font-bold">
                                    {session.profiles?.display_name || "N/A"}
                                  </p>
                                </div>
                                <div>
                                  <p className="text-gray-500 text-[10px] uppercase font-bold">
                                ...
                                <div>
                                  ...
                                  <p className="text-gray-300">
                                    {session.profiles?.platforms?.join(", ") ||
                                      "None"}
                                  </p>
                                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="bg-black border border-gray-800 rounded-xl p-5 h-[500px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-800 pr-2">
                            {cmsPages.map((page) => (
                              <div
                                key={page.id}
                                className="bg-[#111] border border-gray-800 rounded-lg p-4 mb-3"
                              >
                          ...
                          <div className="bg-black border border-gray-800 rounded-xl p-5">
                            <h4 className="text-sm font-bold text-gray-300 mb-4 uppercase tracking-widest">
                              Add New Page
                            </h4>
                            <form
                              onSubmit={(e) => {
                            ...
                            >
                              <input
                                ...
                                required
                                className="bg-[#111] border border-gray-800 rounded-lg p-3 text-sm focus:outline-none focus:border-purple-500 text-white w-full"
                              />
                              <input
                                name="slug"
                                type="text"
                                placeholder="URL Slug (e.g. refund-policy)"
                                required
                                className="bg-[#111] border border-gray-800 rounded-lg p-3 text-sm focus:outline-none focus:border-purple-500 text-white w-full"
                              />
                              <textarea
                                ...
                                required
                                className="bg-[#111] border border-gray-800 rounded-lg p-3 text-sm focus:outline-none focus:border-purple-500 text-white w-full min-h-[150px]"
                              ></textarea>
                              <button
                                type="submit"
                                className="w-full bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 rounded-lg transition-colors"
                      ...
                      <div className="bg-[#111] border border-gray-800 rounded-xl p-6 flex flex-col gap-6">
                          ...
                          <table className="w-full text-left text-sm text-gray-400">
                            <thead className="bg-[#1a1a1a] text-xs uppercase font-bold border-b border-gray-800">
                              <tr>
                                ...
                                <th className="px-4 py-3">
                                  {t[language].originalPrice}
                                </th>
                                <th className="px-4 py-3">
                                  {t[language].discountPrice}
                                </th>
                                <th className="px-4 py-3">
                                  {t[language].featured}
                                </th>
                            ...
                            <tbody className="divide-y divide-gray-800">
                              {productList.map((item, index) => (
                                ...
                                >
                                  ...
                                  <td
                                    className={`px-4 py-3 font-mono ${item.discountPrice ? "line-through text-gray-500" : "text-green-400"}`}
                                  >
                                    ${item.sellingPrice}
                                  </td>
                                  ...
                                  <td className="px-4 py-3">
                                    {item.isFeatured ? (
                                      <span className="bg-purple-500/10 text-purple-400 px-2 py-1 rounded text-[10px] uppercase font-bold tracking-wider">
                                        Featured
                                      </span>
                                    ) : (
                                      "-"
                                  ...
                                  <td className="px-4 py-3">
                                    <button
                                      ...
                                      className="bg-purple-600/20 text-purple-400 hover:bg-purple-600 hover:text-white px-3 py-1 rounded transition-colors text-xs font-bold border border-purple-500/30"
                                    >
                                      Apply Offer
                                    </button>
                                  </td>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="flex flex-col gap-4 bg-black border border-gray-800 rounded-xl p-5 h-[400px] overflow-y-auto">
                            ...
                            <input
                              type="text"
                              placeholder="Link To Category (Optional)"
                              value={newPromotion.linkToCategory}
                              onChange={(e) =>
                                setNewPromotion({
                                  ...newPromotion,
                                  linkToCategory: e.target.value,
                                })
                              }
                              className="w-full bg-[#111] border border-gray-800 rounded-lg p-3 text-sm focus:outline-none focus:border-purple-500 text-white"
                            />
                            ...
                            <button
                                ...
                                if (
                                  ...
                                  try {
                                    ...
                                    const body = {
                                      ...
                                      image_url: newPromotion.imageUrl,
                                      link_to_category:
                                        newPromotion.linkToCategory,
                                      active: true,
                                    };
                                    ...
                                    if (!success) {
                                      setPromotions([
                                        ...promotions,
                                        {
                                          ...
                                          linkToCategory: body.link_to_category,
                                          active: true,
                                        },
                                      ]);
                                    }
                              ...
                              className="w-full bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 rounded-lg transition-colors mt-auto"
                            >
                              {t[language].addBanner}
                            </button>
                          </div>
                          <div className="bg-black border border-gray-800 rounded-xl p-5 h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-800 pr-2">
                            {promotions.map((promo) => (
                              <div
                                key={promo.id}
                                className={`bg-[#111] border border-gray-800 rounded-lg p-4 mb-3 ${!promo.active ? "opacity-50" : ""}`}
                              >
                                <div className="flex justify-between items-start mb-2">
                                  ...
                                  <div className="flex gap-2">
                                    <button
                                      ...
                                      className={`${promo.active ? "text-green-500 hover:text-green-400" : "text-gray-500 hover:text-gray-400"}`}
                                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="flex flex-col gap-4 bg-black border border-gray-800 rounded-xl p-5 h-[500px] overflow-y-auto">
                            ...
                            <input
                              ...
                              className="w-full bg-[#111] border border-gray-800 rounded-lg p-3 text-sm focus:outline-none focus:border-purple-500 text-white uppercase"
                            />
                            <input
                              type="number"
                                ...
                                if (
                                  ...
                                  setPromoCodes([
                                    ...promoCodes,
                                    {
                                      ...
                                      expiryDate:
                                        newPromoCode.expiryDate || "2099-12-31",
                                      usageLimit:
                                        parseInt(newPromoCode.usageLimit) ||
                                        1000,
                                      usedCount: 0,
                                      active: true,
                                    },
                          ...
                          <div className="bg-black border border-gray-800 rounded-xl p-5 h-[500px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-800 pr-2">
                            {promoCodes.map((code) => (
                              <div
                                key={code.id}
                                className={`bg-[#111] border border-gray-800 rounded-lg p-4 mb-3 ${!code.active ? "opacity-50" : ""}`}
                              >
                                <div className="flex justify-between items-start mb-2">
                                  <h5 className="font-bold text-purple-400 text-lg tracking-widest">
                                    {code.code}
                                  </h5>
                                  <div className="flex gap-2">
                                    <button
                                      ...
                                      className={`${code.active ? "text-green-500 hover:text-green-400" : "text-gray-500 hover:text-gray-400"}`}
                                    >
                                      <CheckCircle2 className="w-4 h-4" />
                                    </button>
                                    <button
          ...
          <>
            {/* Navbar */}
            <nav
              className={`h-16 md:h-20 w-full px-4 md:px-8 flex items-center justify-between border-b border-white/5 bg-[#0a0a0c]/90 backdrop-blur-2xl z-20 flex-shrink-0 sticky top-0 shadow-[0_5px_40px_rgba(0,0,0,0.4)] transition-all duration-300 ${isMobileSearchOpen ? "bg-gradient-to-b from-purple-900/10 to-[#0a0a0c]" : ""}`}
            >
              {/* Mobile Search Overlay */}
              <div
                className={`absolute inset-0 px-4 flex items-center md:hidden transition-all duration-300 ease-out z-30 ${isMobileSearchOpen ? "opacity-100 translate-y-0 pointer-events-auto scale-100" : "opacity-0 -translate-y-4 pointer-events-none scale-95"}`}
                dir={language === "ar" ? "rtl" : "ltr"}
              >
                <div className="relative w-full flex items-center shadow-[0_0_20px_rgba(168,85,247,0.15)] rounded-full">
                  <Search
                    className={`absolute ${language === "ar" ? "right-4" : "left-4"} w-4 h-4 text-purple-400`}
                  />
                  <input
                    type="text"
                  ...
                  <button
                    ...
                    className={`absolute ${language === "ar" ? "left-2" : "right-2"} p-2.5 text-gray-400 hover:text-white rounded-full bg-black/20 hover:bg-black/50 transition-colors`}
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              <div
                className={`flex w-full items-center justify-between transition-all duration-300 ${isMobileSearchOpen ? "opacity-0 md:opacity-100 pointer-events-none md:pointer-events-auto blur-sm md:blur-none" : "opacity-100 pointer-events-auto blur-none"}`}
              >
                <div className="flex items-center flex-none gap-6 md:gap-10">
                  <div
                    className="text-xl md:text-2xl font-black tracking-tighter text-white cursor-pointer hidden md:flex items-center"
                    onClick={() => {
                      setActiveTab("store");
                      setActiveCategory(null);
                  ...
                  {/* Mobile Logo Only */}
                  <div
                    className="text-lg font-black tracking-tighter text-white cursor-pointer md:hidden flex items-center gap-2"
                    onClick={() => {
                      setActiveTab("store");
                      setActiveCategory(null);
                    }}
                  >
                    <div className="bg-gradient-to-br from-purple-500 to-indigo-600 text-white w-8 h-8 flex items-center justify-center rounded-lg shadow-[0_0_15px_rgba(147,51,234,0.4)]">
                      L
                    </div>
                    <span className="bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400">
                      LUDEX<span className="text-purple-400">STORE</span>
                    </span>
                  </div>
                    ...
                    <button
                      ...
                      className={
                        ...
                        activeCategory !== "Subscriptions"
                          ...
                          : "hover:text-purple-400 transition-colors duration-300 min-h-[44px]"
                      }
                    >
                      {t[language].store}
                    </button>
                ...
                <div className="flex flex-1 items-center justify-end gap-2 md:gap-6">
                  ...
                  <div
                    className="md:hidden flex items-center justify-center p-2 text-gray-300 hover:text-white cursor-pointer bg-gray-800/40 rounded-full border border-gray-700/50 hover:bg-gray-800/80 transition-colors"
                    onClick={() => {
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div
                      className={`relative cursor-pointer hover:text-purple-400 text-gray-400 transition-colors flex items-center justify-center min-h-[44px] min-w-[44px] hidden md:flex ${cartAnimating ? "animate-bounce-cart" : ""}`}
                      onClick={() => setActiveTab("cart")}
                    >
                      <ShoppingBag className="w-5 h-5" />
                      {cart.length > 0 && (
                        <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 border-2 border-black rounded-full shadow-[0_0_8px_#ef4444] text-[8px] flex items-center justify-center font-bold text-white transition-all transform hover:scale-110">
                          {cart.length}
                        </span>
                      )}
                    </div>
                      ...
                      {isLoggedIn ? (
                        <>
                          <div
                            ...
                            className="w-9 h-9 md:w-10 md:h-10 bg-gradient-to-br from-purple-700 to-purple-900 rounded-full border-2 border-purple-500/50 cursor-pointer overflow-hidden hover:border-purple-400 transition-all flex items-center justify-center font-bold text-white uppercase select-none shadow-[0_0_15px_rgba(168,85,247,0.3)]"
                          >
                              ...
                              <>
                                ...
                                >
                                  ...
                                  <button
                                    ...
                                    className={`w-full text-left px-4 py-2 text-gray-300 hover:bg-white/5 hover:text-white transition-colors flex items-center gap-2`}
                                  >
                                  ...
                                  {userProfile?.role === "ADMIN" && (
                                    <button
                                      ...
                                      className={`w-full text-left px-4 py-2 text-purple-400 hover:bg-purple-900/20 font-bold transition-colors flex items-center gap-2`}
                                    >
                                      <Shield className="w-4 h-4" /> Ludex HQ
                                      Portal
                                    </button>
                                  )}
                                  ...
                                  <button
                                    ...
                                    className={`w-full text-left px-4 py-2 text-red-400 hover:bg-red-900/20 transition-colors flex items-center gap-2`}
                                  >
            ...
            <AnimatePresence>
              {isMobileMenuOpen && (
                <div
                  className="fixed inset-0 z-[1000] md:hidden flex"
                  dir={language === "ar" ? "rtl" : "ltr"}
                >
                  ...
                  >
                    ...
                    <div className="p-5 border-b border-gray-800/40 flex justify-between items-center bg-[#111115]/50">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-xl font-black tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-fuchsia-500">
                          LUDEX
                        </span>
                      </div>
                      <button
                        onClick={() => setIsMobileMenuOpen(false)}
                        className="text-gray-400 hover:text-white p-2 flex items-center justify-center bg-gray-800/30 hover:bg-gray-800/80 rounded-full transition-colors"
                      >
                        <X className="w-5 h-5" />
                      </button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-6 custom-scrollbar">
                      ...
                      <div className="relative group">
                        <Search
                          className={`absolute ${language === "ar" ? "right-3.5" : "left-3.5"} top-1/2 -translate-y-1/2 text-gray-500 w-4 h-4 group-focus-within:text-purple-400 transition-colors`}
                        />
                        <input
                          type="text"
                          placeholder={t[language].search}
                      ...
                      {isLoggedIn && userProfile && (
                        <div className="bg-gradient-to-br from-[#1c1c1f] to-[#121214] border border-gray-800/60 rounded-2xl p-4 flex gap-4 items-center shadow-lg relative overflow-hidden">
                          ...
                          <img
                            src={
                              userProfile?.avatar_url ||
                              `https://api.dicebear.com/7.x/avataaars/svg?seed=${userProfile?.display_name || userProfile?.email}&backgroundColor=4c1d95`
                            }
                            className="w-12 h-12 rounded-xl border border-gray-700 object-cover bg-black relative z-10"
                          />
                      ...
                      <div className="flex flex-col gap-1.5">
                        <p className="text-[10px] uppercase font-black text-gray-600 tracking-widest mb-1 px-3">
                          {t[language].menu}
                        </p>
                        ...
                        <button
                          ...
                          className={`group flex items-center justify-between px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === "store" && !activeCategory ? "bg-purple-600 border border-purple-500 text-white shadow-[0_0_15px_rgba(168,85,247,0.3)]" : "bg-transparent border border-transparent text-gray-300 hover:bg-gray-800/40 hover:text-white"}`}
                        >
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${activeTab === "store" && !activeCategory ? "bg-white/20" : "bg-gray-800/80 group-hover:bg-purple-500/20 group-hover:text-purple-400"}`}
                            >
                        ...
                        {isLoggedIn && (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${activeTab === "user_dashboard" && userDashboardTab === "orders" ? "bg-white/20" : "bg-gray-800/80 group-hover:bg-purple-500/20 group-hover:text-purple-400"}`}
                              >
                        ...
                        <button
                          ...
                          className={`group flex items-center justify-between px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === "cart" ? "bg-purple-600 border border-purple-500 text-white shadow-[0_0_15px_rgba(168,85,247,0.3)]" : "bg-transparent border border-transparent text-gray-300 hover:bg-gray-800/40 hover:text-white"}`}
                        >
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${activeTab === "cart" ? "bg-white/20" : "bg-gray-800/80 group-hover:bg-purple-500/20 group-hover:text-purple-400"}`}
                            >
                              <ShoppingBag className="w-4 h-4" />
                            </div>
                            <span className="text-start">
                          ...
                          {cart.length > 0 && (
                            <span className="bg-white text-purple-700 font-black text-[10px] px-2 py-0.5 rounded-full">
                              {cart.length}
                            </span>
                          )}
                        </button>
                        ...
                        {isLoggedIn && userProfile?.role === "ADMIN" && (
                          <button
                            ...
                            className={`group flex items-center justify-between px-4 py-3 rounded-xl text-sm font-bold transition-all mt-2 ${activeTab === "admin" ? "bg-purple-600 border border-purple-500 text-white shadow-[0_0_15px_rgba(168,85,247,0.3)]" : "bg-purple-900/10 border border-purple-900/30 text-purple-400 hover:bg-purple-900/30 hover:text-purple-300"}`}
                          >
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${activeTab === "admin" ? "bg-white/20" : "bg-purple-500/20 group-hover:bg-purple-500/40"}`}
                              >
                                <Shield className="w-4 h-4" />
                              </div>
                              <span className="text-start">
                      ...
                      <div className="flex flex-col gap-1.5 mt-2">
                        ...
                        {CATEGORIES.map((cat, idx) => {
                          const catName =
                            cat.name === "PC Game Keys"
                              ? t[language].pcKeys
                              : cat.name === "Console Subs"
                                ? t[language].consoleSubs
                                : cat.name === "In-game Currency"
                                  ? t[language].inGameCurrency
                                  : cat.name === "Software"
                                    ? t[language].software
                                    : cat.name;
                          return (
                            ...
                            >
                              <div className="flex items-center gap-3 w-full">
                                <div
                                  className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors flex-shrink-0 ${activeCategory === cat.name ? "bg-white/20" : "bg-gray-800/50 group-hover:bg-purple-500/20 group-hover:text-purple-400"}`}
                                >
                    ...
                    <div className="p-5 border-t border-gray-800/40 bg-[#0a0a0c]/80 backdrop-blur-md flex flex-col gap-4">
                      <div className="flex bg-[#111116] p-1.5 rounded-xl border border-gray-800/80 shadow-inner">
                        <button
                          ...
                          className={`flex-1 py-1.5 text-sm font-bold rounded-lg transition-all ${language === "en" ? "bg-purple-600 text-white shadow-[0_0_10px_rgba(168,85,247,0.4)]" : "text-gray-400 hover:text-white"}`}
                        >
                          En
                        </button>
                        <button
                          ...
                          className={`flex-1 py-1.5 text-sm font-bold rounded-lg transition-all ${language === "ar" ? "bg-purple-600 text-white shadow-[0_0_10px_rgba(168,85,247,0.4)]" : "text-gray-400 hover:text-white"}`}
                        >
                          عربي
                        </button>
                      </div>
                      ...
                      {!isLoggedIn ? (
                        ...
                        <button
                          ...
                          className="w-full py-3.5 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 rounded-xl text-red-500 font-bold flex items-center justify-center gap-2 transition-colors"
                        >
                          <LogOut className="w-4 h-4" />{" "}
                        ...
                        >
                          {activeCategory === cat.name && (
                            <motion.div
                              ...
                              className="absolute inset-0 bg-purple-500/20 md:ltr:border-l-2 md:rtl:border-r-2 border-purple-500 rounded-lg z-0"
                              initial={{ opacity: 0 }}
                              animate={{ opacity: 1 }}
                              exit={{ opacity: 0 }}
                              transition={{
                          ...
                          <div className="relative z-10 flex items-center gap-3 w-full">
                            <cat.icon
                              className={`w-4 h-4 ${activeCategory === cat.name ? "text-purple-400 opacity-100" : "opacity-70 group-hover:text-purple-400 group-hover:opacity-100"} transition-all`}
                            />
                            <span>
                              {cat.name === "PC Game Keys"
                                ? t[language].pcKeys
                ...
                <AnimatePresence mode="wait">
                  {activeTab === "store" && (
                    ...
                    >
                        ...
                        {!activeCategory && !searchQuery && (
                          ...
                          >
                            {(() => {
                              ...
                              const activeBanners = promotions
                                ...
                                .map((b: any) => ({
                                  ...
                                  link: b.linkToCategory,
                                }));
                              const topVisited = [...publicProducts].sort(
                                (a: any, b: any) =>
                                  (b.views_count || 0) - (a.views_count || 0),
                              )[0];
                              const topSelling = [...publicProducts].sort(
                                (a: any, b: any) =>
                              ...
                              const currentSlide =
                                slides[heroSlideIdx % slides.length];
                      ...
                      <AnimatePresence>
                        {!activeCategory &&
                          ...
                          publicProducts.filter(
                            (p) => p.discountPrice || p.isFeatured,
                          ).length > 0 && (
                            <motion.div
                              initial={{
                                opacity: 0,
                              ...
                              <div className="flex gap-4 overflow-x-auto pb-4 pt-2 snap-x snap-mandatory scrollbar-thin scrollbar-thumb-purple-900/50">
                                {publicProducts
                                  ...
                                  .map((product) => (
                                    <div
                                      key={product.id}
                                      className="min-w-[280px] w-[280px] snap-start bg-[#0a0a0a] border border-purple-900/30 rounded-xl overflow-hidden relative group hover:border-purple-500/50 transition-all hover:shadow-[0_0_20px_rgba(147,51,234,0.3)] shadow-[0_0_15px_rgba(0,0,0,0.5)] flex flex-col cursor-pointer"
                                      onClick={() => {
                                        const matchingGame = gamesList.find(
                                          (g) =>
                                            String(g.id) === String(product.id),
                                        );
                                        if (matchingGame) {
                                    ...
                                    >
                                      ...
                                      {product.discountPrice && (
                                        <div className="absolute top-3 left-3 z-20 bg-red-600 text-white text-[10px] uppercase font-black px-2 py-1 rounded shadow-[0_0_10px_#dc2626]">
                                          SALE
                                        </div>
                                      )}
                                      {product.isFeatured &&
                                        !product.discountPrice && (
                                          <div className="absolute top-3 left-3 z-20 bg-purple-600 text-white text-[10px] uppercase font-black px-2 py-1 rounded shadow-[0_0_10px_#9333ea]">
                                            FEATURED
                                          </div>
                                        )}
                                      ...
                                      <div className="p-3 sm:p-5 flex flex-col flex-1 z-10 -mt-8 sm:-mt-10">
                                        <p className="text-xs text-purple-400 font-bold uppercase tracking-wider mb-1 drop-shadow-md">
                                          {product.category}
                                        </p>
                                        <h3 className="text-sm sm:text-lg font-black text-white leading-tight mb-1 sm:mb-2 line-clamp-2 sm:truncate">
                                          {product.name}
                                        </h3>
                                        <div className="mt-auto pt-4 flex items-center justify-between">
                                          <div className="flex flex-col">
                                            {product.discountPrice ? (
                                              <>
                                                ...
                                                <span className="text-base sm:text-xl font-black text-green-400 font-mono shadow-green-500/20 drop-shadow-lg">
                                                  ${product.discountPrice}
                                                </span>
                                              </>
                                            ) : (
                                          ...
                                          <button
                                            className="bg-white/10 hover:bg-purple-600 text-white p-2 rounded-lg transition-colors border border-white/10 group-hover:border-purple-500/50"
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              const item = {
                                                id: product.id,
                      ...
                      <div className="lg:hidden relative w-full mb-8">
                        ...
                        <div
                          className={`absolute top-0 bottom-0 ${language === "ar" ? "right-0" : "left-0"} w-4 bg-gradient-to-r ${language === "ar" ? "from-transparent to-[#0a0a0c]" : "from-[#0a0a0c] to-transparent"} z-10 pointer-events-none`}
                        ></div>
                        <div
                          className={`absolute top-0 bottom-0 ${language === "ar" ? "left-0" : "right-0"} w-12 bg-gradient-to-l ${language === "ar" ? "from-transparent to-[#0a0a0c]" : "from-[#0a0a0c] to-transparent"} z-10 pointer-events-none`}
                        ></div>

                        <div
                          className="w-[calc(100%+3rem)] -mx-6 px-6 overflow-x-auto pb-4 pt-1 flex gap-3 snap-x snap-mandatory scroll-px-6 relative z-0 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
                          dir={language === "ar" ? "rtl" : "ltr"}
                        >
                          <button
                          ...
                          {CATEGORIES.map((cat) => {
                            ...
                            return (
                              ...
                              >
                                <cat.icon
                                  className={`w-4 h-4 transition-transform ${activeCategory === cat.name ? "scale-110 opacity-100" : "opacity-70 group-hover:scale-110"}`}
                                />
                                {catName}
                              </button>
                            );
                      ...
                      <div
                        className="flex flex-col md:flex-row md:items-end justify-between mb-6 md:mb-10 gap-5 relative z-10"
                        dir={language === "ar" ? "rtl" : "ltr"}
                      >
                        <div className="text-center md:text-start">
                          <h1 className="text-3xl sm:text-4xl leading-tight font-black tracking-tight">
                            {t[language].discover}{" "}
                        ...
                        <div className="flex gap-2.5 w-full md:w-auto mt-2 md:mt-0">
                          ...
                          >
                            <option
                              value="newest"
                              className="text-gray-200 bg-[#111]"
                            >
                              {t[language].newest}
                            </option>
                            <option
                      ...
                      <AnimatePresence>
                        {isFilterOpen && (
                          <motion.div
                            ...
                            transition={{ duration: 0.3 }}
                            className="mb-6 p-5 bg-[#0a0a0c] border border-purple-900/40 rounded-2xl flex flex-wrap gap-5 shadow-[0_0_20px_rgba(168,85,247,0.1)] relative overflow-hidden"
                            dir={language === "ar" ? "rtl" : "ltr"}
                          >
                            ...
                            <div className="flex flex-col gap-2.5 flex-1 min-w-[200px] relative z-10">
                              ...
                              >
                                ...
                                <option value="pc" className="bg-[#111]">
                                  PC
                                </option>
                                <option value="ps" className="bg-[#111]">
                                  PlayStation
                                </option>
                                <option value="xbox" className="bg-[#111]">
                                  Xbox
                                </option>
                              </select>
                            </div>
                            <div className="flex flex-col gap-2.5 flex-1 min-w-[200px] relative z-10">
                              ...
                              >
                                ...
                                <option value="under20" className="bg-[#111]">
                                  {language === "ar"
                                    ? "أقل من $20"
                                    : "Under $20"}
                                </option>
                                <option value="20to50" className="bg-[#111]">
                                  $20 - $50
                                </option>
                                <option value="over50" className="bg-[#111]">
                                  {language === "ar"
                                    ? "أكثر من $50"
                                    : "Over $50"}
                                </option>
                        ...
                        <AnimatePresence mode="popLayout">
                          {isLoadingStore ? (
                            Array.from({ length: 6 }).map((_, i) => (
                              <div
                                key={i}
                                className="bg-[#151515] border border-gray-800 rounded-xl flex flex-col h-full animate-skeleton overflow-hidden"
                              >
                            ...
                            filteredGames
                              ...
                              .map((game, index) => (
                                <motion.div
                                  ...
                                  className="group bg-[#08080a] border border-white/5 hover:border-purple-500/30 rounded-[1.25rem] flex flex-col h-full transition-all duration-500 overflow-hidden cursor-pointer relative shadow-lg"
                                  onClick={() => handleOpenGameDetail(game.id)}
                                  dir={language === "ar" ? "rtl" : "ltr"}
                                >
                                  {/* Hover Glow Behind */}
                                  <div className="absolute inset-0 bg-gradient-to-t from-purple-600/0 via-purple-600/0 to-purple-600/0 group-hover:from-purple-900/20 group-hover:via-black/20 group-hover:to-black/0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"></div>

                                  <div className="relative aspect-[3/4] overflow-hidden bg-black w-full shadow-inner">
                                    <img
                                      src={game.image}
                                      ...
                                      alt={game.title}
                                    />
                                    <div className="absolute inset-0 bg-gradient-to-t from-[#08080a] via-[#08080a]/40 to-transparent opacity-90 transition-opacity duration-300"></div>

                                    {/* Top Badges */}
                                    <div className="absolute top-3 left-3 right-3 flex justify-between items-start pointer-events-none z-10">
                                      <div className="flex flex-col gap-1.5">
                                        <span className="px-2.5 py-1 bg-black/40 backdrop-blur-md border border-white/10 rounded-lg text-[10px] font-black uppercase text-gray-300 tracking-wider shadow-sm">
                                          {game.type}
                                        </span>
                                        {game.originalPrice && (
                                          <span className="w-fit px-2 py-0.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded text-[10px] font-bold shadow-md shadow-purple-500/20">
                                            -
                                            {Math.round(
                                              (1 -
                                                game.price /
                                      ...
                                      {(!game.stock || game.stock === 0) && (
                                        <span className="bg-red-500/80 backdrop-blur shadow-lg shadow-red-500/20 text-white px-2 py-1 rounded text-[10px] font-bold uppercase">
                                  ...
                                  <div className="px-4 pb-4 pt-0 flex flex-col justify-between flex-1 relative z-20 -mt-10">
                                    <div className="flex items-center justify-between mb-1.5 pointer-events-none">
                                      <div className="flex items-center gap-1.5">
                                        {game.category.includes("Console") ||
                                        game.type.includes("PS5") ||
                                        game.type.includes("Xbox") ||
                                        game.type.includes("Nintendo") ? (
                                    ...
                                    <h4
                                      className="font-bold text-sm leading-snug text-white mb-4 line-clamp-2 transition-colors group-hover:text-purple-300 min-h-[2.5rem]"
                                      dir="auto"
                                    >
                                      {game.title}
                                    </h4>
                                    ...
                                    <div className="flex items-end justify-between mt-auto w-full group/bottom">
                                      <div className="flex flex-col">
                                        {game.originalPrice ? (
                                          <div className="flex flex-col">
                                            <span className="text-gray-500 text-[11px] line-through decoration-gray-500/50 leading-none mb-0.5 font-medium">
                                              {displayPrice(game.originalPrice)}
                                            </span>
                                            <span className="text-[17px] font-black text-white leading-none font-mono drop-shadow-sm">
                                              {displayPrice(game.price)}
                                            </span>
                                          </div>
                                        ) : (
                                      ...
                                      <button
                                        ...
                                        className="h-10 w-10 sm:group-hover:w-auto relative rounded-xl bg-purple-600/10 hover:bg-purple-600 border border-purple-500/30 hover:border-purple-500 text-purple-400 hover:text-white flex items-center justify-center transition-all duration-300 z-30 overflow-hidden sm:px-0 sm:group-hover:px-4"
                                        title={
                                          t[language].addToCart || "Add to Cart"
                                        }
                                      >
                                        <div className="flex items-center justify-center gap-2 whitespace-nowrap">
                                          <ShoppingCart className="w-4 h-4 flex-shrink-0" />
                                          <span className="text-xs font-bold hidden sm:opacity-0 sm:group-hover:opacity-100 sm:group-hover:inline-block transition-opacity duration-300">
                                            {language === "ar"
                                              ? "إضافة"
                                              : "Add"}
                                          </span>
                  ...
                  {(activeTab === "orders" ||
                    ...
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -10 }}
                      transition={{ duration: 0.2 }}
                      className="max-w-[95%] 2xl:max-w-[1400px] w-full mx-auto flex flex-col items-start gap-8"
                    >
                      <div>
                      ...
                      {orders.length === 0 ? (
                        <div className="w-full p-10 border border-purple-900/30 rounded-2xl bg-black/20 flex flex-col items-center justify-center text-center">
                          ...
                          <button
                            ...
                            className="mt-4 px-4 py-2 bg-purple-600 rounded-lg text-white text-sm font-bold hover:bg-purple-500 transition-colors"
                          >
                            {t[language].goStore}
                          </button>
                        </div>
                        ...
                        <div className="w-full flex flex-col gap-4">
                          {orders.map((order) => {
                            ...
                            return (
                              <div
                                key={order.id}
                                className="w-full bg-[#111] border border-purple-900/30 rounded-xl p-5 flex flex-col md:flex-row gap-6 justify-between md:items-center"
                              >
                                <div className="flex gap-4 items-center">
                                  <img
                                    src={game?.image}
                                    className="w-20 h-20 rounded-lg object-cover border border-purple-900/50"
                                    alt=""
                                  />
                                  <div>
                  ...
                  {activeTab === "cart" && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -10 }}
                      transition={{ duration: 0.2 }}
                      className="max-w-[95%] 2xl:max-w-[1400px] w-full mx-auto flex flex-col gap-6"
                    >
                      ...
                      {cart.length === 0 ? (
                        <div className="w-full p-10 border border-purple-900/30 rounded-2xl bg-[#111] flex flex-col items-center justify-center text-center">
                          ...
                          <button
                            ...
                            className="mt-4 px-6 py-2 bg-purple-600 rounded-lg text-white text-sm font-bold hover:bg-purple-500 transition-colors min-h-[44px] flex items-center justify-center"
                          >
                            {t[language].goStore}
                          </button>
                        </div>
                        ...
                        <div className="flex flex-col lg:flex-row gap-8">
                          <div className="flex-1 flex flex-col gap-4">
                            {cart.map((gameId, index) => {
                              ...
                              return (
                                <div
                                  key={index}
                                  className="flex gap-4 items-center bg-[#111] border border-purple-900/30 p-5 rounded-xl relative group hover:border-purple-500/50 transition-colors"
                                >
                                  <img
                                    src={game?.image}
                                    className="w-16 h-16 sm:w-20 sm:h-20 object-cover rounded-lg border border-purple-900/50"
                                    alt={game?.title}
                                  />
                                  <div className="flex-1">
                                    <h4 className="font-bold text-white text-sm md:text-base leading-tight uppercase tracking-wider">
                                      {game?.title}
                                    </h4>
                                    <p className="text-[10px] sm:text-xs text-gray-500 mt-1 uppercase tracking-widest">
                                      {game?.type}
                                    </p>
                                    <p className="text-purple-400 font-bold mt-2">
                                      {displayPrice(game?.price || 0)}
                                    </p>
                                  </div>
                                  <button
                                    onClick={() =>
                          ...
                          <div
                            className="w-full lg:w-80 bg-[#111] rounded-xl p-6 flex flex-col gap-4 h-fit sticky top-0 border"
                            style={{
                              borderColor: "rgba(176, 38, 255, 0.3)",
                              boxShadow: "0 0 25px rgba(176,38,255,0.1)",
                            }}
                          >
                            <h3 className="font-bold text-lg border-b border-gray-800 pb-3 text-white uppercase tracking-widest">
                              {t[language].summary}
                            </h3>
                            ...
                            <div className="flex flex-col gap-2 border-b border-gray-800 pb-4">
                              <div className="flex gap-2">
                                ...
                                <button
                                  ...
                                  className="bg-purple-600 hover:bg-purple-500 text-white px-3 py-2 rounded-lg text-sm font-bold transition-colors"
                                >
                                  Apply
                                </button>
                              </div>
                            ...
                            {calculateFinalPrice(
                              cart.reduce(
                                (sum, id) =>
                                  sum +
                                  (gamesList.find((g) => g.id === id)?.price ||
                                    0),
                                0,
                              ),
                            ).surchargeApplied > 0 && (
                              <div className="flex justify-between text-sm text-orange-400 font-bold mb-1">
                                <span>
                                  {paymentMethod} Fee (
                                  {calculateFinalPrice(0).surchargePercent}%)
                                </span>
                                <span>
                                  +
                                  {displayPrice(
                        ...
                        <div className="bg-[#111] border border-purple-900/30 rounded-2xl p-6 flex flex-col sm:flex-row items-center sm:items-start gap-6 relative overflow-hidden">
                          ...
                          <div className="relative group">
                            <img
                              ...
                              alt="Avatar"
                              className="w-24 h-24 rounded-full border-2 border-purple-500 shadow-[0_0_15px_rgba(147,51,234,0.3)] relative z-10 object-cover"
                            />
                          ...
                          <div className="flex flex-col items-center sm:items-start text-center sm:text-start relative z-10">
                            ...
                            <div className="flex gap-4">
                              <div className="bg-black border border-gray-800 rounded-lg p-3 text-center min-w-[100px] shadow-inner">
                                <p className="text-2xl font-black text-white drop-shadow-[0_0_5px_rgba(255,255,255,0.5)]">
                                  {orders.length}
                                </p>
                              ...
                              <div className="bg-black border border-green-900/40 rounded-lg p-3 text-center min-w-[100px] shadow-inner">
                                <p className="text-2xl font-black text-green-400 drop-shadow-[0_0_10px_rgba(74,222,128,0.5)]">
                                  {approvedOrdersCount}
                                </p>
                                <p className="text-[10px] text-green-700 font-bold uppercase tracking-widest mt-1">
                                  Verified
                                </p>
                              </div>
                            </div>
                          ...
                          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
                            <div className="flex-1 w-full">
                              <div className="flex items-center gap-4 mb-3">
                                ...
                                <span
                                  className={`px-3 py-1 bg-black border border-gray-800 rounded-full font-bold text-[10px] uppercase tracking-widest ${currentTier.color}`}
                                >
                                  {currentTier.name} Tier
                                </span>
                              </div>
                              <p className="text-sm text-gray-400 mb-6 max-w-md">
                                Earn {globalSettings.xpMultiplier} XP for every
                                $1 spent. Unlock exclusive rewards by reaching
                                higher tiers!
                              </p>
                            ...
                            <button
                              onClick={() =>
                                setToastMessage(
                                  "Exporting Gamer Card... Please wait.",
                                )
                              }
                              className="flex items-center gap-2 px-5 py-2.5 bg-purple-600/20 text-purple-400 border border-purple-500 hover:bg-purple-600 hover:text-white transition-all rounded-lg text-xs font-bold uppercase tracking-widest whitespace-nowrap"
                            >
                                  ...
                                  >
                                    ...
                                    <td className="px-6 py-4 text-right">
                                      {order.status === "Approved" && (
                                        <button
                                          onClick={() =>
                                            setInvoiceToView(order)
                                          }
                                          className="text-xs text-blue-400 font-bold hover:text-blue-300 underline underline-offset-2"
                                        >
                                          View
                                        </button>
                                      )}
                    ...
                    >
                      ...
                      <form
                        onSubmit={async (e) => {
                          ...
                          try {
                            ...
                            if (!uId) {
                              setToastMessage(
                                "Error saving settings: missing user ID.",
                              );
                              return;
                            }
                            const token = await getAuthToken();
                            const res = await fetch(
                              `/api/admin/profiles/${uId}`,
                              {
                                method: "PUT",
                                headers: {
                                  "Content-Type": "application/json",
                      ...
                      >
                          ...
                          <div className="flex flex-wrap gap-2">
                            {[
                              ...
                              <button
                                ...
                                className={`px-4 py-2 rounded-lg text-sm font-bold border transition-colors ${userProfile?.platforms?.includes(plat) ? "bg-purple-600/20 border-purple-500 text-purple-400" : "bg-black border-gray-800 text-gray-400 hover:border-gray-600"}`}
                              >
                                {plat}
                              </button>
                            ))}
                          ...
                          <div className="flex flex-wrap gap-2">
                            {[
                              ...
                              <button
                                ...
                                className={`px-4 py-2 rounded-lg text-sm font-bold border transition-colors ${userProfile?.interests?.includes(int) ? "bg-purple-600/20 border-purple-500 text-purple-400" : "bg-black border-gray-800 text-gray-400 hover:border-gray-600"}`}
                              >
                                {int}
                              </button>
                            ))}
                        ...
                        <div className="flex gap-4 items-center mt-4">
                          <button
                            type="submit"
                            className="bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 px-6 rounded-lg transition-colors shadow-[0_0_15px_rgba(147,51,234,0.2)]"
                          >
                            {t[language].save}
                          </button>
                          <button
                            ...
                            onClick={async () => {
                              if (
                                ...
                                try {
                                  ...
                                  if (user?.userId) {
                                    await fetch(
                                      `/api/public/profiles/${user.userId}`,
                                      { method: "DELETE" },
                                    );
                                  }
                                  await signOut();
                                  ...
                                  setActiveTab("store");
                                  setToastMessage(
                                    "Account scheduled for deletion. It will be permanently removed in 30 days.",
                                  );
                                  setTimeout(() => {
                                    setToastMessage(null);
            ...
            <div
              className="md:hidden fixed bottom-0 left-0 right-0 h-[80px] pb-[safe-area-inset-bottom] bg-[#0a0a0c]/85 backdrop-blur-2xl border-t border-white/5 z-[90] flex items-center justify-around px-2 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]"
              dir={language === "ar" ? "rtl" : "ltr"}
            >
              <button
                onClick={() => {
                ...
                className="relative flex flex-col items-center justify-center w-full h-full group pb-2"
              >
                <div className="relative flex flex-col items-center justify-center h-full pt-1">
                  <Home
                    className={`w-6 h-6 transition-all duration-500 ease-out ${activeTab === "store" ? "text-white scale-110 drop-shadow-[0_0_12px_rgba(255,255,255,0.8)] fill-white/20" : "text-gray-500 group-hover:text-gray-400"}`}
                  />
                  <span
                    className={`text-[10px] mt-1.5 font-bold transition-all duration-300 ${activeTab === "store" ? "text-white" : "text-gray-500"}`}
                  >
                    {t[language].store}
                  </span>
                </div>
              ...
              >
                <div className="relative flex flex-col items-center justify-center h-full pt-1">
                  <ShoppingCart
                    className={`w-6 h-6 transition-all duration-500 ease-out ${cartAnimating ? "animate-bounce-cart" : ""} ${activeTab === "cart" ? "text-white scale-110 drop-shadow-[0_0_12px_rgba(255,255,255,0.8)] fill-white/20" : "text-gray-500 group-hover:text-gray-400"}`}
                  />
                  {cart.length > 0 && (
                    <span className="absolute -top-1 -right-2 min-w-[18px] h-[18px] px-1 bg-red-500 text-white text-[10px] font-black flex items-center justify-center rounded-full shadow-lg border-2 border-[#0a0a0c] animate-in zoom-in duration-300">
                      {cart.length}
                    </span>
                  )}
                  <span
                    className={`text-[10px] mt-1.5 font-bold transition-all duration-300 ${activeTab === "cart" ? "text-white" : "text-gray-500"}`}
                  >
                    {t[language].cart || "Cart"}
                  </span>
                </div>
              </button>
              ...
              >
                <div className="relative flex flex-col items-center justify-center h-full pt-1">
                  {isLoggedIn ? (
                    <ListOrdered
                      className={`w-6 h-6 transition-all duration-500 ease-out ${activeTab === "user_dashboard" && userDashboardTab === "orders" ? "text-white scale-110 drop-shadow-[0_0_12px_rgba(255,255,255,0.8)]" : "text-gray-500 group-hover:text-gray-400"}`}
                    />
                  ) : (
                    <User
                      className={`w-6 h-6 transition-all duration-500 ease-out text-gray-500 group-hover:text-gray-400`}
                    />
                  )}
                  <span
                    className={`text-[10px] mt-1.5 font-bold transition-all duration-300 ${activeTab === "user_dashboard" && userDashboardTab === "orders" ? "text-white" : "text-gray-500"}`}
                  >
                    {isLoggedIn ? t[language].orders : t[language].signIn}
                  </span>
                </div>
              </button>
              ...
              >
                <div className="relative flex flex-col items-center justify-center h-full pt-1">
                  <Menu
                    className={`w-6 h-6 transition-all duration-500 ease-out ${isMobileMenuOpen ? "text-white scale-110 drop-shadow-[0_0_12px_rgba(255,255,255,0.8)]" : "text-gray-500 group-hover:text-gray-400"}`}
                  />
                  <span
                    className={`text-[10px] mt-1.5 font-bold transition-all duration-300 ${isMobileMenuOpen ? "text-white" : "text-gray-500"}`}
                  >
                    Menu
                  </span>
                </div>
          ...
          <div className="bg-[#0a0a0a] border-t md:border border-purple-900/40 rounded-t-3xl md:rounded-xl w-full md:max-w-2xl shadow-[0_-10px_50px_rgba(168,85,247,0.2)] md:shadow-[0_0_50px_rgba(168,85,247,0.2)] flex flex-col max-h-[95vh] md:max-h-[90vh] animate-in slide-in-from-bottom-5 duration-300">
            <div className="p-4 md:p-6 border-b border-gray-800 flex justify-between items-center bg-[#111] flex-shrink-0">
              ...
              <button
                ...
                className="text-gray-500 hover:text-white p-2 min-h-[44px] min-w-[44px] flex items-center justify-center"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
            <div className="p-4 md:p-8 overflow-y-auto flex-1 font-mono">
              <div className="flex justify-between items-start border-b border-gray-800 pb-6 md:pb-8 mb-6 md:mb-8">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-4 max-w-[200px]">
                    support@ludexstore.com
                    <br />
                    0770 123 4567
                  </p>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-sm text-white font-bold">
                    {invoiceToView.invoiceNumber || "INV-XXXX"}
                  </p>
                  <p className="text-xs text-gray-500 uppercase tracking-widest mt-4 mb-1">
                    Date
                  </p>
                  <p className="text-sm text-white font-bold">
              ...
              <table className="w-full text-left text-sm text-gray-300">
                <thead>
                  <tr className="border-b border-gray-800">
                    <th className="pb-3 text-xs uppercase tracking-widest text-gray-500">
                      {t[language].descriptionCol}
                    </th>
                    <th className="pb-3 text-xs uppercase tracking-widest text-gray-500 text-right">
                      {t[language].amount}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b border-gray-800/50">
                    <td className="py-4 font-bold text-white">
                      {
                        gamesList.find((g) => g.id === invoiceToView.gameId)
                          ?.title
                      }
                    </td>
                    <td className="py-4 text-right">
                      {displayPrice(invoiceToView.amount)}
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <div className="mt-8 flex justify-between items-end border-t border-gray-800 pt-6">
                <div className="text-xs text-gray-500 max-w-[250px] leading-relaxed">
                  Payment Method: Zain Cash / FIB (Manual Transfer)
                  <br />
                ...
                <div className="text-right w-64">
                  ...
                  {invoiceToView.discountApplied ? (
                    <div className="flex justify-between items-center mb-2 text-green-400">
                      ...
                      <span className="text-sm">
                        -{displayPrice(invoiceToView.discountApplied)}
                      </span>
                    </div>
                  ) : null}
                  <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-800">
                    ...
                    <span className="text-2xl font-black text-purple-400">
                      {displayPrice(
                        invoiceToView.finalPrice !== undefined
                          ? invoiceToView.finalPrice
                          : invoiceToView.amount,
                      )}
                    </span>
                  </div>
                </div>
      ...
      {publishModal.isOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          ></div>
          <div
            className="bg-gradient-to-b from-[#151515] to-[#0a0a0a] border border-purple-900/50 rounded-2xl w-full max-w-lg p-8 relative shadow-[0_0_80px_rgba(147,51,234,0.3)] animate-in zoom-in-95 duration-300"
            dir={language === "ar" ? "rtl" : "ltr"}
          >
            ...
            <div className="flex justify-between items-center mb-2 relative z-10">
              ...
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-full bg-black/50 text-gray-400 hover:text-white hover:bg-purple-900/50 transition-colors border border-gray-800 hover:border-purple-500/50"
              >
                ✕
              </button>
            </div>
            ...
            <div className="mb-6 bg-black/80 p-4 rounded-xl border border-purple-900/30 flex items-center gap-3 relative z-10 shadow-inner">
              ...
              <p
                className="text-md text-white font-mono break-all line-clamp-2"
                dir="ltr"
              >
                {publishModal.subscription?.name ||
                  publishModal.subscription?.title}
              </p>
            </div>
                ...
                try {
                  await fetch("/api/admin/products", {
                    ...
                    body: JSON.stringify({
                      ...
                      supplier:
                        publishModal.subscription?.supplier ||
                        publishModal.subscription?.account_username ||
                        "Local",
            ...
            >
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-2 text-start">
                    {t[language].costPriceOpt}
                  </label>
                  <div className="relative">
                    <span
                      className={
                        "absolute top-1/2 -translate-y-1/2 text-purple-500 font-bold " +
                    ...
                    <input
                      ...
                      className={
                        "w-full bg-[#111] border border-gray-800 rounded-xl py-3 text-sm focus:outline-none focus:border-purple-500 text-white transition-all focus:bg-purple-900/10 focus:shadow-[0_0_15px_rgba(168,85,247,0.2)] " +
                        (language === "ar" ? "pr-8 pl-3" : "pl-8 pr-3")
                      }
                      placeholder={t[language].placeholder0}
                    />
                  </div>
                </div>
                <div>
                  <label className="block text-[10px] uppercase tracking-widest text-purple-400 font-bold mb-2 text-start">
                    {t[language].sellPriceRequired}
                  </label>
                  <div className="relative">
                    <span
                      className={
                    ...
                    <input
                      ...
                      step="0.01"
                      required
                      value={publishModal.sellingPrice}
                      onChange={(e) =>
                        setPublishModal({
                          ...publishModal,
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-[#111] border border-gray-800 rounded-xl p-3 text-sm focus:outline-none focus:border-purple-500 text-white transition-all focus:bg-purple-900/10 cursor-pointer"
                  >
                ...
                <div>
                  <label className="block text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-2 text-start">
                    {t[language].typeRegInput}
                  </label>
                  <input
                    dir="ltr"
                    type="text"
                    value={publishModal.type}
                    onChange={(e) =>
                      setPublishModal({ ...publishModal, type: e.target.value })
                    }
                    className="w-full bg-[#111] border border-gray-800 rounded-xl p-3 text-sm focus:outline-none focus:border-purple-500 text-white text-start transition-all focus:bg-purple-900/10 focus:shadow-[0_0_15px_rgba(168,85,247,0.2)]"
                    placeholder={t[language].placeholderReg}
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-2 text-start">
                  {t[language].coverImgOpt}
                </label>
                <input
                  dir="ltr"
                  type="text"
                  value={publishModal.image}
                  onChange={(e) =>
                    setPublishModal({ ...publishModal, image: e.target.value })
                  }
                  className="w-full bg-[#111] border border-gray-800 rounded-xl p-3 text-sm focus:outline-none focus:border-purple-500 text-white text-start transition-all focus:bg-purple-900/10 focus:shadow-[0_0_15px_rgba(168,85,247,0.2)]"
                  placeholder={t[language].leaveEmpty}
                />
              </div>
              ...
              <div className="flex items-center gap-3 mt-6">
                <button
                  ...
                  className="flex-1 bg-black border border-gray-800 text-white py-3.5 rounded-xl hover:bg-gray-900 hover:border-gray-600 transition-all font-bold text-sm"
                >
                  {t[language].cancelText}
                </button>
                <button
                  type="submit"
                  className="flex-1 bg-gradient-to-r from-purple-600 to-purple-800 text-white py-3.5 rounded-xl hover:from-purple-500 hover:to-purple-700 transition-all font-black text-sm uppercase tracking-widest shadow-[0_0_20px_rgba(168,85,247,0.4)] hover:shadow-[0_0_30px_rgba(168,85,247,0.6)]"
                >
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-[#111] border border-gray-800 rounded-xl w-full max-w-md p-6 relative shadow-2xl">
            <h3 className="text-xl font-bold text-white mb-4 capitalize">
              {crudModal.mode} {crudModal.table}
            </h3>
            <form
              onSubmit={(e) => {
                e.preventDefault();
                ...
                crudModal.fields.forEach((f) => {
                  ...
                  if (f.type === "number") data[f.name] = Number(val);
                    ...
                    data[f.name] = fd.get(f.name) === "on" ? true : false;
              ...
              {crudModal.fields.map((f) => (
                <div key={f.name}>
                  ...
                  {f.type === "select" ? (
                    <select
                      name={f.name}
                      defaultValue={crudModal.item?.[f.name]}
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <input
                        type="checkbox"
                        name={f.name}
                        defaultChecked={crudModal.item?.[f.name]}
                    ...
                    <input
                      ...
                      name={f.name}
                      defaultValue={
                        f.type === "password" && crudModal.mode === "edit"
                          ? "MASKED"
                          : crudModal.item?.[f.name]
                      }
                      className="w-full bg-black border border-gray-800 rounded p-2 text-white font-mono mt-1 focus:outline-none focus:border-purple-500"
                      required
                    />
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-[#111] border border-purple-900/50 rounded-xl w-full max-w-md p-6 relative shadow-2xl">
            <h3 className="text-xl font-bold text-white mb-4">
              Apply Offer: {promoModal.product.name}
            </h3>
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="text-white font-mono">
                  ${promoModal.product.sellingPrice}
                </p>
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase">
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      const token = await getAuthToken();
                      const res = await fetch(
                        `/api/admin/products/${promoModal.product.id}`,
                        {
                          method: "PUT",
                          headers: {
                            "Content-Type": "application/json",
                      ...
                      if (res.ok) {
                        ...
                        // Refresh list
                        const resList = await fetch("/api/admin/products", {
                          headers: { Authorization: `Bearer ${token}` },
                        });
                        ...
                        if (publicRes.ok)
                          setPublicProducts(await publicRes.json());
                      }
                    } catch (e) {
                      console.error(e);
                    }
          ...
          (() => {
            ...
            return (
              <motion.div
                ...
                exit={{ opacity: 0 }}
                transition={{ duration: 0.3 }}
                className="fixed inset-0 bg-black/80 backdrop-blur-md z-[100] flex items-center justify-center p-0 lg:p-4"
              >
                <motion.div
                  ...
                  className={`bg-[#151515] border border-purple-900/40 rounded-none lg:rounded-2xl w-full h-full lg:h-auto lg:max-h-[90vh] lg:max-w-4xl overflow-hidden flex flex-col md:flex-row relative shadow-[0_0_80px_rgba(147,51,234,0.2)] ${isGameDetailLoading ? "animate-skeleton" : ""}`}
                >
                  <button
                    ...
                    style={
                      language === "ar" ? { left: "1rem", right: "auto" } : {}
                    }
                  >
                    <X className="w-5 h-5" />
                  </button>
                    ...
                    <>
                      <div className="w-full h-64 md:h-auto md:w-1/2 relative bg-gradient-to-br from-purple-900/20 to-black border-b md:border-b-0 md:border-r border-purple-900/30 flex-shrink-0">
                        ...
                        <div className="absolute inset-0 p-8 flex flex-col justify-end">
                          <div className="bg-black/80 backdrop-blur px-3 py-1.5 w-fit rounded text-xs font-bold uppercase border border-purple-500/30 text-purple-400 mb-3">
                            {game.type}
                          </div>
                      ...
                      <div className="w-full md:w-1/2 flex-1 p-6 md:p-8 overflow-y-auto scrollbar-thin scrollbar-thumb-purple-900/50 flex flex-col gap-6 pb-24 md:pb-8">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="text-sm font-bold text-gray-500 tracking-wider uppercase mb-1">
                              {game.category}
                            </p>
                        ...
                        <div>
                          <p className="text-gray-300 text-sm leading-relaxed whitespace-pre-wrap">
                            {game.description ||
                              "Get your digital key instantly after payment. Full warranty included. Easy activation."}
                          </p>
                        </div>
                              ...
                              gameReviews.map((review, idx) => (
                                ...
                                >
                                  <div className="flex items-center justify-between mb-2">
                                    <div
                                      className="flex items-center gap-3 cursor-pointer hover:bg-black/40 p-1 -ml-1 rounded transition-colors group"
                                      onClick={() => {
                                        if (review.user_id)
                                          setViewedProfileId(review.user_id);
                                      }}
                                    >
                                      {review.profiles?.avatar_url ? (
                                        <img
                                          src={review.profiles.avatar_url}
                                          className="w-8 h-8 rounded-full border border-purple-500/30 object-cover group-hover:border-purple-500"
                                        />
                                      ) : (
      ...
      <AnimatePresence>
        {viewedProfile && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.2 }}
            className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4 shadow-2xl overflow-y-auto"
          ...
          >
            <motion.div
              ...
              transition={{ type: "spring", stiffness: 300, damping: 30 }}
              className="bg-[#050505] border border-purple-900/40 rounded-2xl w-full max-w-md p-6 relative shadow-[0_0_50px_rgba(147,51,234,0.15)] mt-4 mb-4"
              onClick={(e) => e.stopPropagation()}
            >
              <button
                onClick={() => setViewedProfile(null)}
                className="absolute top-4 right-4 text-gray-500 hover:text-white bg-black hover:bg-purple-900/40 p-2 rounded-full transition-all"
              >
                <X className="w-5 h-5" />
              </button>
              <div className="flex flex-col items-center mt-4">
                <div className="relative">
                  ...
                  <img
                    src={
                      ...
                      `https://api.dicebear.com/7.x/avataaars/svg?seed=${viewedProfile?.display_name || viewedProfile?.email}&backgroundColor=4c1d95`
                    }
                    alt="Avatar"
                    className="w-24 h-24 rounded-full border-2 border-purple-500 relative z-10 shadow-[0_0_15px_rgba(147,51,234,0.3)] object-cover bg-[#111]"
                  />
                </div>
                <h3 className="text-xl font-black text-white mt-4 mb-1 uppercase tracking-widest">
                ...
                <div className="w-full flex gap-3 mb-6">
                  <div className="flex-1 bg-[#111] border border-gray-800 rounded-xl p-3 text-center">
                    <p className="text-lg font-black text-white">
                      {viewedProfile?.stats?.totalOrders || 0}
                    </p>
                  ...
                  <div className="flex-1 bg-[#111] border border-green-900/30 rounded-xl p-3 text-center">
                    <p className="text-lg font-black text-green-400">
                      {viewedProfile?.stats?.approvedOrders || 0}
                    </p>
                    <p className="text-[10px] uppercase text-green-700 font-bold tracking-wider">
                      Verified
                    </p>
                  </div>
                </div>
                ...
                <div className="w-full space-y-4 text-left">
                  {viewedProfile?.platforms &&
                    viewedProfile.platforms.length > 0 && (
                      <div>
                        <p className="text-[10px] uppercase text-gray-500 tracking-widest mb-2 font-bold text-center">
                          Gaming Platforms
                        </p>
                        <div className="flex flex-wrap justify-center gap-2">
                          {viewedProfile.platforms.map((p: string) => (
                            <span
                              key={p}
                              className="px-3 py-1 bg-[#111] border border-gray-800 rounded text-xs font-bold text-gray-300"
                            >
                              {p}
                            </span>
                          ))}
                      ...
                      <div className="pt-2">
                        ...
                        <div className="flex flex-wrap justify-center gap-2">
                          {viewedProfile.interests.map((p: string) => (
                            <span
                              key={p}
                              className="px-3 py-1 bg-purple-900/10 border border-purple-900/30 text-purple-400 rounded text-xs font-bold"
                            >
                              {p}
                            </span>
                          ))}
        ...
        {viewedProfileId && (
          ...
          >
            <motion.div
              ...
              exit={{ y: 20, scale: 0.95 }}
              transition={{ type: "spring", stiffness: 300, damping: 30 }}
              className="bg-[#111] border border-purple-500/30 w-full max-w-sm rounded-2xl p-6 relative"
            >
              <button
                onClick={() => {
              ...
              {!viewedProfileData ? (
                ...
                <div className="flex flex-col items-center text-center">
                  {viewedProfileData.avatar_url ? (
                    <img
                      src={viewedProfileData.avatar_url}
                      className="w-24 h-24 rounded-full border-2 border-purple-500 object-cover shadow-[0_0_20px_rgba(147,51,234,0.3)] mb-4"
                    />
                  ) : (
                  ...
                  <h3 className="text-xl font-bold text-white mb-1">
                    {viewedProfileData.display_name}
                  </h3>
                  <p className="text-sm text-gray-400 mb-4">
                    Joined{" "}
                    {new Date(
                      viewedProfileData.created_at || Date.now(),
                    ).toLocaleDateString()}
                  </p>

                  {viewedProfileData.platforms &&
                    viewedProfileData.platforms.length > 0 && (
                      <div className="w-full flex flex-wrap gap-2 justify-center mb-4">
                        {viewedProfileData.platforms.map((plat: string) => (
                          <span
                            key={plat}
                            className="px-2 py-1 bg-black border border-purple-900/50 text-gray-300 text-[10px] uppercase font-bold rounded"
                          >
                            {plat}
                          </span>
                        ))}
                  ...
                  {viewedProfileData.interests &&
                    viewedProfileData.interests.length > 0 && (
                      <div className="w-full text-left bg-black/50 border border-gray-800 p-3 rounded-xl">
                        <p className="text-xs text-purple-400 font-bold mb-2 uppercase tracking-widest">
                          {language === "ar" ? "الاهتمامات" : "Interests"}
                        </p>
                        <div className="flex flex-wrap gap-2">
      ...
      <AnimatePresence>
        {isCheckoutModalOpen && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.2 }}
            className="fixed inset-0 z-[100] flex flex-col items-center justify-end md:justify-center bg-black/80 backdrop-blur-sm"
          >
            <motion.div
              initial={{ y: "100%" }}
              animate={{ y: 0 }}
              exit={{ y: "100%" }}
              transition={{ type: "spring", damping: 25, stiffness: 200 }}
              className="bg-[#1e1e24] border-t md:border border-purple-900/40 rounded-t-3xl md:rounded-2xl w-full md:max-w-lg shadow-[0_-10px_50px_rgba(147,51,234,0.3)] md:shadow-[0_0_50px_rgba(147,51,234,0.3)] flex flex-col overflow-hidden max-h-[95vh] md:max-h-[90vh]"
            >
                    ...
                    {paymentMethodsList
                      ...
                      .map((method) => (
                        ...
                        >
                          ...
                          {method.surcharge_percentage > 0 && (
                            <span
                              className={`text-[10px] uppercase mt-1 ${paymentMethod === method.name ? "text-purple-200" : "text-orange-400"}`}
                            >
                              +{method.surcharge_percentage}% FEE
                            </span>
                          )}
                ...
                <div className="space-y-4 mb-6">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">
                      Transfer Details ({paymentMethod})
                    </label>
                    <div className="bg-[#2a2a32] border border-gray-700 rounded-lg p-4 text-sm font-mono text-white flex justify-between items-center shadow-inner">
                      <span>
                        {paymentMethodsList.find(
                          (p) => p.name === paymentMethod,
                        )?.accountDetails || "Select a payment method"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {activePromoCode && (
                      <div className="mb-2 bg-purple-500/10 border border-purple-500/20 text-purple-400 p-2 rounded text-xs font-bold flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4" />
                        Promo Code ({activePromoCode.code}):{" "}
                    ...
                    {calculateFinalPrice(
                      cart.reduce(
                        (sum, id) =>
                          sum +
                          (gamesList.find((g) => g.id === id)?.price || 0),
                        0,
                      ),
                    ).surchargeApplied > 0 && (
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-[#2a2a32] border border-gray-700 rounded-lg p-4 text-base font-bold focus:outline-none focus:border-purple-500 text-white shadow-inner"
                      />
                      {isEligibleForLoyaltyDiscount && (
                        <span className="absolute right-4 top-4 text-gray-500 line-through text-sm">
                          {displayPrice(
                            cart.reduce(
                              (sum, id) =>
                                sum +
                ...
                <div className="border-2 border-dashed border-purple-500/60 rounded-xl p-8 flex flex-col items-center justify-center gap-3 hover:border-purple-400 hover:bg-purple-900/20 transition-all cursor-pointer group relative bg-[#2a2a32]">
                  ...
                  {receiptFile ? (
                    <>
                      ...
                      <p className="text-sm text-green-400 font-bold max-w-full truncate px-4">
                        {receiptFile.name}
                      </p>
                    ...
                    <>
                      ...
                      <p className="text-[10px] text-purple-400 font-medium uppercase tracking-widest mt-1">
                        JPEG, PNG or PDF limit 5MB
                      </p>
                    </>
                  )}
                </div>
      ...
      {/* Chat Widget */}
      <div
        className={`fixed bottom-24 md:bottom-10 ${language === "ar" ? "left-4 md:left-8 items-start" : "right-4 md:right-8 items-end"} z-[110] md:z-50 flex flex-col pointer-events-none`}
      >
        <AnimatePresence>
          {isChatOpen && (
            <motion.div
            ...
            >
              <div className="bg-gradient-to-r from-purple-900/40 to-indigo-900/40 border-b border-gray-800 p-4 flex justify-between items-center flex-shrink-0 relative overflow-hidden">
                ...
                <button
                  ...
                  className="text-gray-400 hover:text-white transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center relative z-10 hover:bg-white/5 rounded-full"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
                ...
                {!isLoggedIn ? (
                  <div className="h-full flex flex-col items-center justify-center text-center px-6 gap-3">
                    ...
                    <p className="text-sm text-gray-400 font-medium leading-relaxed">
                      Please log in to start a conversation with our support
                      team.
                    </p>
                  </div>
                  ...
                  chatHistory.map((msg, i) => {
                    ...
                    const showHeader =
                      i === 0 || chatHistory[i - 1].sender_id !== msg.sender_id;

                    return (
                      <div
                        key={msg.id}
                        className={`flex ${isMe ? "justify-end" : "justify-start"} flex-col gap-1`}
                      >
                        {!isMe && showHeader && (
                          <span className="text-[10px] font-bold text-gray-500 ml-1 mt-2 uppercase tracking-wide flex items-center gap-1">
                            ...
                            {msg.sender?.display_name || "Support"}
                          </span>
                        )}
                        <div
                          dir="auto"
                        ...
                        >
                          {msg.content?.startsWith("[Product Inquiry ID:") ||
                          msg.content?.startsWith("[Product Inquiry:") ? (
                            (() => {
                              ...
                              if (
                                ...
                                const gameIdMatch = msg.content.match(
                                  /\[Product Inquiry ID: (.*?)\]/,
                                );
                              ...
                              if (!game)
                                return (
                                  <div className="flex flex-col gap-2 min-w-[180px]">
                                    <p className="opacity-80 text-[10px] font-bold text-white/70 uppercase tracking-widest">
                                      {language === "ar"
                                        ? "استفسار عن منتج"
                                        : "Product Inquiry"}
                                    </p>
                                    <div className="bg-black/30 border border-red-500/20 rounded-lg p-2 flex gap-2 items-center">
                                      <div className="w-10 h-10 flex items-center justify-center bg-red-900/20 rounded border border-red-500/30 text-red-500 text-xs">
                                        ?
                                      </div>
                                      <div className="flex-1 min-w-0">
                                        <p className="font-bold text-gray-500 truncate text-xs">
                                          Product Unavailable
                                        </p>
                                      </div>
                                    </div>
                                ...
                                <div className="flex flex-col gap-2 min-w-[180px]">
                                  ...
                                  >
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <p className="text-white/80 font-black text-[10px]">
                                        {displayPrice(game.price)}
                                      </p>
                                    </div>
                                  </div>
                        ...
                        {(() => {
                          ...
                          const adminReaders =
                            msg.read_by_profiles?.filter(
                              (p: any) =>
                                p.role === "ADMIN" && p.id !== msg.sender_id,
                            ) || [];
              ...
              {isLoggedIn && (
                <form
                  ...
                  className="p-3 border-t border-purple-900/30 bg-black/50 flex gap-2 flex-shrink-0"
                >
                  <input
                    type="text"
                    id="chatMessage"
