...
const enInsert = `
    ...
    catSelect: "Category", typeRegInput: "Type / Region", coverImgOpt: "Cover Image URL",
    cancelText: "Cancel", confirmPub: "Publish Now", placeholder0: "e.g. 10.00",
    placeholderReg: "e.g. Global, TR",
`;

const arInsert = `
    publishItemTitle: "نشر العنصر", publishSubtitle: "أنت على وشك نشر العنصر التالي في المتجر:",
...
const oldModalEnd = appText.indexOf('{publishModal.isOpen && (', oldModalStart) > -1 ? appText.indexOf(`)}`, appText.indexOf(`{publishModal.isOpen && (`)) : -1;

// Actually I'll use replace with a regex to grab the publishModal block.
              ...
              setTimeout(() => setToastMessage(null), 3000);
               ...
               <div className="grid grid-cols-2 gap-4">
                 <div>
                    ...
                    <div className="relative">
                      <span className="absolute top-1/2 -translate-y-1/2 left-3 text-purple-500 font-bold">$</span>
                      <input dir="ltr" type="number" step="0.01" value={publishModal.costPrice} onChange={e => setPublishModal({...publishModal, costPrice: e.target.value})} className="w-full bg-[#111] border border-gray-800 rounded-xl py-3 pl-8 pr-3 text-sm focus:outline-none focus:border-purple-500 text-white transition-all focus:bg-purple-900/10 focus:shadow-[0_0_15px_rgba(168,85,247,0.2)]" placeholder={t[language].placeholder0} />
                    </div>
                 </div>
                 <div>
                    ...
                    <div className="relative">
                      <span className="absolute top-1/2 -translate-y-1/2 left-3 text-purple-500 font-bold">$</span>
                      <input dir="ltr" type="number" step="0.01" required value={publishModal.sellingPrice} onChange={e => setPublishModal({...publishModal, sellingPrice: e.target.value})} className="w-full bg-[#111] border border-gray-800 rounded-xl py-3 pl-8 pr-3 text-sm focus:outline-none focus:border-purple-500 text-white transition-all focus:bg-purple-900/10 focus:shadow-[0_0_15px_rgba(168,85,247,0.2)]" placeholder={t[language].placeholder0} />
                    </div>
                 </div>
               </div>
               ...
               <div className="grid grid-cols-2 gap-4">
                 <div>
                    ...
                    <select value={publishModal.category} onChange={e => setPublishModal({...publishModal, category: e.target.value})} className="w-full bg-[#111] border border-gray-800 rounded-xl p-3 text-sm focus:outline-none focus:border-purple-500 text-white transition-all focus:bg-purple-900/10 cursor-pointer appearance-none">
